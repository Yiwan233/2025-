...
代码名称: P1_V2.5_JSON_Config_Workflow
版本: 2.5
日期: 2025-05-12 (根据当前时间)
描述: 从JSON文件 (config_params.json) 加载全局参数和位点特异性参数。
      实现了完整的数据加载、Stutter概率化评估、以及特征工程的完整工作流。
      输出包含真实等位基因置信度(CTA)的处理后峰数据(df_processed_peaks)
      和基于此计算的新特征集(df_features_v2_5)。
"""
# --- 基础库与机器学习库导入 ---
import pandas as pd
import re
...
PLOTS_DIR = os.path.join(DATA_DIR, 'prob1_plots_v2.5_json_eval')
...
try:
    ...
    SATURATION_THRESHOLD = float(global_params_json.get("saturation_threshold_rfu", 30000.0))
    SIZE_TOLERANCE_BP = float(global_params_json.get("size_tolerance_bp", 0.5))
    STUTTER_CV_HS_GLOBAL = float(global_params_json.get("stutter_cv_hs_global_n_minus_1", 0.25))
    GLOBAL_AT = float(global_params_json.get("default_at_unknown_dye", 50.0))
    TRUE_ALLELE_CONFIDENCE_THRESHOLD = float(global_params_json.get("true_allele_confidence_threshold", 0.5))
    ...
    if not MARKER_PARAMS:
        ...
        for marker, params_val in MARKER_PARAMS.items():
            if "Dye" in params_val:
                MARKER_PARAMS[marker]["Dye"] = str(params_val["Dye"]).upper()
            # 确保Stutter参数存在，即使为空也初始化结构，避免后续KeyError
            if "n_minus_1_Stutter" not in MARKER_PARAMS[marker]:
                 MARKER_PARAMS[marker]["n_minus_1_Stutter"] = {"SR_model_type": "N/A", "SR_m": 0.0, "SR_c": 0.0, "RS_max_k": 0.0}


except FileNotFoundError: print(f"严重错误: 配置文件 '{CONFIG_FILE_PATH}' 未找到。请确保文件存在且路径正确。脚本终止。"); exit()
except json.JSONDecodeError as e: print(f"严重错误: 配置文件 '{CONFIG_FILE_PATH}' 格式错误，无法解析: {e}。请检查JSON语法。脚本终止。"); exit()
except Exception as e_config: print(f"严重错误: 加载配置文件时发生未知错误: {e_config}。脚本终止。"); exit()
...
def calculate_peak_confidence_v2_5(locus_peaks_df_input, marker_name, marker_params_dict,
    ...
    for _, peak_row in locus_peaks_df.iterrows(): # 使用 iterrows 以便访问原始索引（如果需要）
    ...
    # 使用 .rename() 来处理 reset_index 后的列名 (如果需要保留旧索引)
    # 当前逻辑是 drop=True，因为主要在排序后的 peaks_df 上操作
    ...
    for i in range(len(peaks_df)): # pc_row is potential stutter
        ...
        max_score_this_pc_is_stutter_from_any_parent = 0.0 # 为每个pc_row初始化
        ...
        for j in range(len(peaks_df)): # pp_row is potential parent
            ...
            if debug_log_list_param is not None and is_pos_match_n_minus_1:
                 debug_entry = {
                    'Marker': marker_name, 'Parent_Allele': pp_row['Allele'], 'Parent_Size': pp_row['Size'], 
                    'Parent_Height_Corrected': pp_row['Height'], 'Candidate_Stutter_Allele': pc_row['Allele'], 
                    'Candidate_Stutter_Size': pc_row['Size'], 'Candidate_Stutter_Height_Corrected': pc_row['Height'],
            ...
            if is_pos_match_n_minus_1:
                ...
                if sr_obs > sr_max_n_minus_1: score_stutter_for_this_parent_pair = 0.0
                    ...
                    e_sr = np.nan; parent_an = pp_row['Allele_Numeric'] # 初始化e_sr为NaN
                    if pd.notna(parent_an) and parent_an >= 0:
                        model_type = stutter_params_n_minus_1.get('SR_model_type'); 
                        m_val = stutter_params_n_minus_1.get('SR_m', np.nan); # 如果缺失，设为NaN
                        c_val = stutter_params_n_minus_1.get('SR_c', np.nan); # 如果缺失，设为NaN
                    ...
                    if pd.isna(e_sr): # 如果E[SR]未能计算出来
                        score_stutter_for_this_parent_pair = 0.0 # 不能评估，则认为不是stutter
                        if debug_entry: debug_entry['Expected_SR'] = np.nan; # 记录为NaN
                    else:
...
for sample_file_name, group_data_per_sample in df_prob1.groupby('Sample File'):
    ...
    for marker_name_actual, locus_data_from_groupby in group_data_per_sample.groupby('Marker'):
        ...
        processed_locus_df = calculate_peak_confidence_v2_5( 
