...
代码名称: P1_V2.4_Final_Stutter_Workflow
版本: 2.4
日期: 2025-05-11
描述: 最终整合版，修正了reset_index的参数错误，包含完整的Stutter评估逻辑
      和特征工程框架。使用用户提供的详细MARKER_PARAMS结构。
...
feature_filename_prob1 = os.path.join(DATA_DIR, 'prob1_features_v2.4.csv') # 更新版本
...
STUTTER_CV_HS_GLOBAL = 0.25 # 全局n-1 Stutter峰高的假设变异系数
GLOBAL_AT = 50
DYE_AT_VALUES = {
    'BLUE': 75, 'GREEN': 101, 'YELLOW': 60, 'RED': 69, 'PURPLE': 56,
    'ORANGE': 50, 'UNKNOWN': GLOBAL_AT # 确保Dye键为大写
}

# MARKER_PARAMS 字典 (核心！请您用您最终确认的、从文献整理的参数完整替换以下内容)
# 结构: 'MarkerName': {'L_repeat': int, 'Dye': str (大写),
#                     'n_minus_1_Stutter': {'SR_model_type': str, 'SR_m': float, 'SR_c': float, 'RS_max_k': float},
#                     (可选) 'n_plus_1_Stutter': {...} }
# CV_Hs 将使用全局的 STUTTER_CV_HS_GLOBAL
MARKER_PARAMS = {
    ...
    "D1S1656": {"L_repeat": 4, "Dye": "BLUE",   "n_minus_1_Stutter": {"SR_model_type": "LUS Regression",  "SR_m": 0.00604,   "SR_c": 0.00132,   "RS_max_k": 0.22}},
    ...
    "TH01":    {"L_repeat": 4, "Dye": "YELLOW", "n_minus_1_Stutter": {"SR_model_type": "LUS Regression",  "SR_m": 0.00185,  "SR_c": 0.00801,   "RS_max_k": 0.10}},
    ...
    "D22S1045":{"L_repeat": 3, "Dye": "RED",    "n_minus_1_Stutter": {"SR_model_type": "LUS Regression",  "SR_m": 0.01528,  "SR_c": -0.1354,   "RS_max_k": 0.25}},
...
def get_allele_numeric_v2_4(allele_val_str):
    try: return float(allele_val_str)
        ...
        if isinstance(allele_val_str, str):
            ...
            if allele_upper == 'OL': return -3.0 # 为OL也分配一个特殊数值
        return np.nan
...
def calculate_peak_confidence_v2_4(locus_peaks_df_input, marker_name, marker_params_dict,
    ...
    params = marker_params_dict.get(marker_name, 
                                    {'L_repeat': 0, 'Dye': locus_peaks_df['Dye'].iloc[0].upper() if not locus_peaks_df.empty and 'Dye' in locus_peaks_df.columns else 'UNKNOWN', 
    ...
    # *** 修正点：使用 .rename() ***
    peaks_df = pd.DataFrame(candidate_peaks_list).sort_values(by='Height', ascending=False).reset_index().rename(columns={'index':'sorted_index_temp'})
    ...
    if l_repeat == 0 or stutter_params_n_minus_1.get('SR_model_type') == 'N/A' or pd.isna(stutter_params_n_minus_1.get('RS_max_k', np.nan)) or stutter_params_n_minus_1.get('RS_max_k', 0.0) == 0.0:
        ...
        if 'sorted_index_temp' in peaks_df.columns: peaks_df.drop(columns=['sorted_index_temp'], inplace=True)
    ...
    if 'sorted_index_temp' in peaks_df.columns: peaks_df.drop(columns=['sorted_index_temp'], inplace=True)
    if 'original_peak_index' in peaks_df.columns: # 如果保留了原始索引，确保只返回需要的列
         return peaks_df[processed_peaks_output_cols + ['original_peak_index']] # original_peak_index was just added to candidate_peaks_list
...
# ... (主循环和函数调用，与V2.4一致) ...
# <<<<< 在此插入或确保 V2.4 中 Step 2 的完整、可成功运行的代码，调用 calculate_peak_confidence_v2_4 >>>>>
...
for sample_file_name, group_data_per_sample in df_prob1.groupby('Sample File'):
    ...
    for marker_name_actual, locus_data_from_groupby in group_data_per_sample.groupby('Marker'):
        ...
        processed_locus_df = calculate_peak_confidence_v2_4(locus_peaks_for_filter_df, marker_name_actual, MARKER_PARAMS, DYE_AT_VALUES, GLOBAL_AT, SATURATION_THRESHOLD, SIZE_TOLERANCE_BP, STUTTER_CV_HS_GLOBAL, stutter_debug_log_list)
...
print(f"\n--- 步骤 3: 特征工程 (版本 2.4) ---")
# ... (您需要将之前的步骤3代码复制到此处，并确保它使用 df_processed_peaks 和 CTA 列生成 df_features_v2_4) ...
# <<<<< 在此插入或确保 V2.3 中 Step 3 的完整、可成功运行的代码, 输出 df_features_v2_4 >>>>>
df_features_v2_4 = pd.DataFrame() # 占位符
# ----- 假设 Step 3 执行完毕，df_features_v2_4 已生成 -----
# 为了脚本能运行，我将加入V2.3的Step3代码
try:
    ...
    df_features_v2_4 = pd.DataFrame(index=all_sample_files_index_df.index)
    df_features_v2_4['NoC_True'] = all_sample_files_index_df['NoC_True']
    ...
    if not df_effective_alleles.empty:
        ...
        }.items(): df_features_v2_4[col] = series.reindex(df_features_v2_4.index)
        
        grouped_heights_eff = df_effective_alleles.groupby('Sample File')['Height']
        df_features_v2_4['avg_peak_height'] = grouped_heights_eff.mean().reindex(df_features_v2_4.index)
        df_features_v2_4['std_peak_height'] = grouped_heights_eff.std().reindex(df_features_v2_4.index)
    else: 
        for col in ['max_allele_per_sample', 'total_alleles_per_sample', 'avg_alleles_per_marker', 
            ...
            df_features_v2_4[col] = 0
            
    df_features_v2_4.fillna(0, inplace=True)
    df_features_v2_4.reset_index(inplace=True)

    print("\n--- 特征工程完成 ---")
    print(f"最终特征数据框 df_features_v2_4 维度: {df_features_v2_4.shape}")
    print_df_in_chinese(df_features_v2_4.head(), col_map=COLUMN_TRANSLATION_MAP, title="新特征数据框 (df_features_v2_4) 前5行")
except Exception as e_feat_eng_v2_4: print(f"严重错误: 在特征工程阶段发生错误: {e_feat_eng_v2_4}"); import traceback; traceback.print_exc();

# --- 步骤 4 & 5: 模型评估 (您需要从V1.15复制并适配这里的代码，使用df_features_v2_4) ---
print(f"\n--- 步骤 4 & 5: 模型评估 (版本 2.4) ---")
# <<<<< 在此插入或确保 V1.15 中 Step 4 和 Step 5 的完整、可成功运行的代码, 输入为 df_features_v2_4 >>>>>
print("--- 步骤 4 & 5 (框架) 完成 ---")
