...
代码名称: P1_V2.12_Simplified_SR_Handling
版本: 2.12
日期: 2025-05-12
描述: 进一步“清除限制”，简化了对Observed_SR的处理。
     - 保留了一个较高的硬上限 RS_MAX_K_HARD_CAP。
     - 对于低于此上限的峰对，Stutter分数主要由Z-score决定，
       移除了基于RS_MAX_K_IDEAL和RS_MAX_K_ACCEPTABLE_MAX的区间惩罚因子。
     - 沿用动态AT和V2.8的核心Stutter计算逻辑。
...
# --- Stutter比率阈值定义 ---
# RS_MAX_K_IDEAL 和 RS_MAX_K_ACCEPTABLE_MAX 不再用于施加惩罚，但可以保留定义以供参考或未来使用
# RS_MAX_K_IDEAL = 0.28 
# RS_MAX_K_ACCEPTABLE_MAX = 0.50
RS_MAX_K_HARD_CAP = 0.65    # 硬上限，超过此值几乎不可能是Stutter (Stutter Score = 0)
# PENALTY_FACTOR_EXTENDED_SR 和 PENALTY_FACTOR_VERY_HIGH_SR_MODEL 不再使用
...
PLOTS_DIR = os.path.join(DATA_DIR, 'prob1_plots_v2.12_simplified_sr')
processed_peaks_filename = os.path.join(DATA_DIR, 'prob1_processed_peaks_v2.12.csv')
stutter_debug_log_filename = os.path.join(DATA_DIR, 'prob1_stutter_debug_log_v2.12.csv')
feature_filename_prob1 = os.path.join(DATA_DIR, 'prob1_features_v2.12.csv')
...
# --- 从JSON文件加载全局参数 (版本 2.12) ---
print(f"\n--- 从JSON文件加载全局参数 (版本 2.12) ---")
config = {}
DEFAULT_SATURATION_THRESHOLD = 30000.0
DEFAULT_SIZE_TOLERANCE_BP = 0.5
DEFAULT_STUTTER_CV_HS_GLOBAL = 0.35 # 假设你认为0.35是目前较好的CV值
...
# --- 核心：峰处理与Stutter评估函数 (版本 2.12 - 简化SR处理) ---
def calculate_peak_confidence_v2_12(locus_peaks_df_input,
    ...
    for i in range(len(peaks_df)): 
        ...
        for j in range(len(peaks_df)): 
            ...
            if is_pos_match_n_minus_1:
                ...
                # MODIFIED: Simplified SR handling - primarily rely on Z-score after a hard cap
                if sr_obs > RS_MAX_K_HARD_CAP: # Still use a hard cap for extremely high SR
                    current_stutter_score_for_pair = 0.0 
                else: 
                    ...
                    if pd.notna(e_sr):
                        ...
                        if e_hs > 1e-6 :
                            ...
                            if sigma_hs > 1e-6:
                                ...
                                # MODIFICATION: Removed SR-interval based penalties. Score is now base_score.
                                current_stutter_score_for_pair = base_score
                                
                                # Optional: Re-introduce a very simple penalty if sr_obs is much larger than e_sr
                                # if pd.notna(e_sr) and e_sr > 1e-6:
                                #     if sr_obs > e_sr * 2.5 : current_stutter_score_for_pair *= 0.2 # Stronger penalty
...
# --- 主逻辑 ---
# (步骤 1: 数据加载与NoC提取 - 与V2.11.1一致，此处省略以保持简洁，请从V2.11.1复制)
# ...
# 步骤 1: 数据加载与NoC提取
print(f"\n--- 步骤 1：数据加载与 NoC 提取 (版本 2.12) ---")
df_prob1 = None
try:
...
# --- 步骤 2: 峰识别与Stutter概率化评估 (使用动态AT和简化的SR处理) ---
print(f"\n--- 步骤 2: 峰识别与Stutter概率化评估 (版本 2.12) ---")
...
for sample_file_name, group_data_per_sample in df_prob1.groupby('Sample File'):
    ...
    for marker_name_actual, locus_data_from_groupby in group_data_per_sample.groupby('Marker'):
        ...
        processed_locus_df = calculate_peak_confidence_v2_12( # 调用新版函数
...
# --- 步骤 3: 特征工程 (版本 2.12) ---
print(f"\n--- 步骤 3: 特征工程 (版本 2.12) ---")
df_features_v2_12 = pd.DataFrame() # MODIFIED: DataFrame name
...
try:
    ...
    df_features_v2_12 = pd.DataFrame(index=all_sample_files_index_df.index)
    df_features_v2_12['NoC_True'] = all_sample_files_index_df['NoC_True']
    ...
    if not df_effective_alleles.empty and \
       ...
       'Allele' in df_effective_alleles.columns:
        ...
        df_features_v2_12['max_allele_per_sample'] = grouped_by_sample_eff.max().reindex(df_features_v2_12.index)
        df_features_v2_12['total_alleles_per_sample'] = grouped_by_sample_eff.sum().reindex(df_features_v2_12.index)
        df_features_v2_12['avg_alleles_per_marker'] = grouped_by_sample_eff.mean().reindex(df_features_v2_12.index)
        df_features_v2_12['markers_gt2_alleles'] = grouped_by_sample_eff.apply(lambda x: (x > 2).sum()).reindex(df_features_v2_12.index)
        df_features_v2_12['markers_gt3_alleles'] = grouped_by_sample_eff.apply(lambda x: (x > 3).sum()).reindex(df_features_v2_12.index)
        df_features_v2_12['markers_gt4_alleles'] = grouped_by_sample_eff.apply(lambda x: (x > 4).sum()).reindex(df_features_v2_12.index)
        
        if 'Height' in df_effective_alleles.columns:
            grouped_heights_eff = df_effective_alleles.groupby('Sample File')['Height']
            df_features_v2_12['avg_peak_height'] = grouped_heights_eff.mean().reindex(df_features_v2_12.index)
            df_features_v2_12['std_peak_height'] = grouped_heights_eff.std().reindex(df_features_v2_12.index)
        else:
            print("警告: df_effective_alleles 缺少 'Height' 列，峰高相关特征无法计算。")
    
    df_features_v2_12.fillna(0, inplace=True)
    df_features_v2_12.reset_index(inplace=True)

    print("\n--- 特征工程完成 ---")
    print(f"最终特征数据框 df_features_v2_12 维度: {df_features_v2_12.shape}")
    print_df_in_chinese(df_features_v2_12.head(), col_map=COLUMN_TRANSLATION_MAP, title="新特征数据框 (df_features_v2_12) 前5行")
    
    try:
        df_features_v2_12.to_csv(feature_filename_prob1, index=False, encoding='utf-8-sig')
        print(f"特征数据已保存到: {feature_filename_prob1}")
    except Exception as e:
        print(f"错误: 保存特征数据失败: {e}")

except Exception as e_feat_eng_v2_12:
    print(f"严重错误: 在特征工程阶段发生错误: {e_feat_eng_v2_12}")
    import traceback
    traceback.print_exc();
print("--- 步骤 3 完成 ---")
...
# --- 步骤 4 & 5: 模型评估 (框架) ---
print(f"\n--- 步骤 4 & 5: 模型评估 (版本 2.12) ---")
# <<<<< 在此插入或确保你的模型训练和评估代码, 输入为 df_features_v2_12 >>>>>
print("--- 步骤 4 & 5 (框架) 完成 ---")

print(f"\n脚本 {os.path.basename(__file__)} (版本 2.12) 执行完毕。")
