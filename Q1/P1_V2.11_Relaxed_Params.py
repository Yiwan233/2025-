...
代码名称: P1_V2.11_Relaxed_Params
版本: 2.11.1 (修正了 FALLBACK_AT_FOR_LOW_SIGNAL_RFU 未定义的错误)
日期: 2025-05-12
描述: 根据“Stutter峰识别改进与验证方案”阶段三，对Stutter分析脚本中的
     SR软边界阈值和惩罚因子进行了“松弛”调整，以期提高对真实Stutter的检出。
     - 调整了 RS_MAX_K_IDEAL, PENALTY_FACTOR_EXTENDED_SR 等参数。
     - 核心Stutter计算逻辑沿用V2.8的软边界SR处理。
...
# --- “松弛”后的Stutter比率阈值和惩罚因子定义 ---
RS_MAX_K_IDEAL = 0.28
RS_MAX_K_ACCEPTABLE_MAX = 0.50
RS_MAX_K_HARD_CAP = 0.65
PENALTY_FACTOR_EXTENDED_SR = 0.5
PENALTY_FACTOR_VERY_HIGH_SR_MODEL = 0.15

# --- 新增回：动态AT计算相关参数 (这些值需要你根据数据进行优化和验证) ---
DYNAMIC_AT_PERCENTAGE = 0.10 
MINIMUM_RELIABLE_PEAK_FOR_DYNAMIC_AT_CALC_RFU = 75 
FALLBACK_AT_FOR_LOW_SIGNAL_RFU = 30 # <--- 此变量之前缺失，现已添加
...
PLOTS_DIR = os.path.join(DATA_DIR, 'prob1_plots_v2.11_relaxed_params')
processed_peaks_filename = os.path.join(DATA_DIR, 'prob1_processed_peaks_v2.11.csv')
stutter_debug_log_filename = os.path.join(DATA_DIR, 'prob1_stutter_debug_log_v2.11.csv')
feature_filename_prob1 = os.path.join(DATA_DIR, 'prob1_features_v2.11.csv')
...
# --- 核心：峰处理与Stutter评估函数 (版本 2.11.1 - 使用调整后的软边界参数) ---
def calculate_peak_confidence_v2_11(locus_peaks_df_input,
                                   sample_file_name_param, 
                                   marker_name, 
                                   marker_params_dict,
                                   current_sample_at_val, # 动态AT值
                                   sat_threshold,
                                   size_tolerance, 
...
# --- 步骤 2: 峰识别与Stutter概率化评估 (使用动态AT和调整后的软边界参数) ---
print(f"\n--- 步骤 2: 峰识别与Stutter概率化评估 (版本 2.11.1) ---")
...
for sample_file_name, group_data_per_sample in df_prob1.groupby('Sample File'):
    ...
    current_sample_at = FALLBACK_AT_FOR_LOW_SIGNAL_RFU # 此处确保 FALLBACK_AT_FOR_LOW_SIGNAL_RFU 已定义
    ...
    for marker_name_actual, locus_data_from_groupby in group_data_per_sample.groupby('Marker'):
        ...
        processed_locus_df = calculate_peak_confidence_v2_11( 
...
# --- 步骤 3: 特征工程 (版本 2.11.1) ---
print(f"\n--- 步骤 3: 特征工程 (版本 2.11.1) ---")
df_features_v2_11 = pd.DataFrame() 
...
try:
    ...
    df_features_v2_11 = pd.DataFrame(index=all_sample_files_index_df.index)
    df_features_v2_11['NoC_True'] = all_sample_files_index_df['NoC_True']
    ...
    if not df_effective_alleles.empty and \
       ...
       'Allele' in df_effective_alleles.columns:
        ...
        df_features_v2_11['max_allele_per_sample'] = grouped_by_sample_eff.max().reindex(df_features_v2_11.index)
        df_features_v2_11['total_alleles_per_sample'] = grouped_by_sample_eff.sum().reindex(df_features_v2_11.index)
        df_features_v2_11['avg_alleles_per_marker'] = grouped_by_sample_eff.mean().reindex(df_features_v2_11.index)
        df_features_v2_11['markers_gt2_alleles'] = grouped_by_sample_eff.apply(lambda x: (x > 2).sum()).reindex(df_features_v2_11.index)
        df_features_v2_11['markers_gt3_alleles'] = grouped_by_sample_eff.apply(lambda x: (x > 3).sum()).reindex(df_features_v2_11.index)
        df_features_v2_11['markers_gt4_alleles'] = grouped_by_sample_eff.apply(lambda x: (x > 4).sum()).reindex(df_features_v2_11.index)
        
        if 'Height' in df_effective_alleles.columns:
            grouped_heights_eff = df_effective_alleles.groupby('Sample File')['Height']
            df_features_v2_11['avg_peak_height'] = grouped_heights_eff.mean().reindex(df_features_v2_11.index)
            df_features_v2_11['std_peak_height'] = grouped_heights_eff.std().reindex(df_features_v2_11.index)
        else:
            print("警告: df_effective_alleles 缺少 'Height' 列，峰高相关特征无法计算。")
    
    df_features_v2_11.fillna(0, inplace=True)
    df_features_v2_11.reset_index(inplace=True)

    print("\n--- 特征工程完成 ---")
    print(f"最终特征数据框 df_features_v2_11 维度: {df_features_v2_11.shape}")
    print_df_in_chinese(df_features_v2_11.head(), col_map=COLUMN_TRANSLATION_MAP, title="新特征数据框 (df_features_v2_11) 前5行")
    
    try:
        df_features_v2_11.to_csv(feature_filename_prob1, index=False, encoding='utf-8-sig')
        print(f"特征数据已保存到: {feature_filename_prob1}")
    except Exception as e:
        print(f"错误: 保存特征数据失败: {e}")

except Exception as e_feat_eng_v2_11:
    print(f"严重错误: 在特征工程阶段发生错误: {e_feat_eng_v2_11}")
    import traceback
    traceback.print_exc();
print("--- 步骤 3 完成 ---")

# --- 步骤 4 & 5: 模型评估 (框架) ---
print(f"\n--- 步骤 4 & 5: 模型评估 (版本 2.11.1) ---")
# <<<<< 在此插入或确保你的模型训练和评估代码, 输入为 df_features_v2_11 >>>>>
print("--- 步骤 4 & 5 (框架) 完成 ---")

print(f"\n脚本 {os.path.basename(__file__)} (版本 2.11.1) 执行完毕。")
