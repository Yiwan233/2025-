...
代码名称: P1_V2.13_No_SR_Hard_Cap
版本: 2.13
日期: 2025-05-12
描述: 实验性修改：在Stutter分析中移除了基于Observed_SR的硬上限过滤 (RS_MAX_K_HARD_CAP)。
     所有位置匹配且模型有效的峰对都将进行完整的Stutter计算，其分数主要由Z-score决定。
...
# --- Stutter比率阈值定义 ---
# RS_MAX_K_HARD_CAP 在此版本中不再用于预过滤，但可以保留定义以供参考
# RS_MAX_K_HARD_CAP = 0.65 
# 其他SR区间惩罚因子也不再使用

# --- 动态AT计算相关参数 (与V2.11/V2.12一致) ---
...
PLOTS_DIR = os.path.join(DATA_DIR, 'prob1_plots_v2.13_no_sr_cap')
processed_peaks_filename = os.path.join(DATA_DIR, 'prob1_processed_peaks_v2.13.csv')
stutter_debug_log_filename = os.path.join(DATA_DIR, 'prob1_stutter_debug_log_v2.13.csv')
feature_filename_prob1 = os.path.join(DATA_DIR, 'prob1_features_v2.13.csv')
...
# --- 核心：峰处理与Stutter评估函数 (版本 2.13 - 移除SR硬上限过滤) ---
def calculate_peak_confidence_v2_13(locus_peaks_df_input,
    ...
    for i in range(len(peaks_df)): 
        ...
        for j in range(len(peaks_df)): 
            ...
            if is_pos_match_n_minus_1:
                ...
                # MODIFICATION: Removed RS_MAX_K_HARD_CAP pre-filter for calculation.
                # All positionally matched peaks (not N/A model type) will attempt calculation.
                ...
                if pd.notna(e_sr):
                    ...
                    if e_hs > 1e-6 :
                        ...
                        if sigma_hs > 1e-6:
                            ...
                            # Optional: A very simple penalty for extremely high SRs if Z-score alone is not enough
                            # if sr_obs > 0.65 : current_stutter_score_for_pair *= 0.01 
                        else: 
...
# --- 步骤 2: 峰识别与Stutter概率化评估 (使用动态AT和移除SR硬上限的计算逻辑) ---
print(f"\n--- 步骤 2: 峰识别与Stutter概率化评估 (版本 2.13) ---")
...
for sample_file_name, group_data_per_sample in df_prob1.groupby('Sample File'):
    ...
    for marker_name_actual, locus_data_from_groupby in group_data_per_sample.groupby('Marker'):
        ...
        processed_locus_df = calculate_peak_confidence_v2_13( # 调用新版函数
...
# --- 步骤 3: 特征工程 (版本 2.13) ---
print(f"\n--- 步骤 3: 特征工程 (版本 2.13) ---")
df_features_v2_13 = pd.DataFrame() # MODIFIED: DataFrame name
...
try:
    ...
    df_features_v2_13 = pd.DataFrame(index=all_sample_files_index_df.index)
    df_features_v2_13['NoC_True'] = all_sample_files_index_df['NoC_True']
    ...
    if not df_effective_alleles.empty and \
       ...
       'Allele' in df_effective_alleles.columns:
        ...
        df_features_v2_13['max_allele_per_sample'] = grouped_by_sample_eff.max().reindex(df_features_v2_13.index)
        df_features_v2_13['total_alleles_per_sample'] = grouped_by_sample_eff.sum().reindex(df_features_v2_13.index)
        df_features_v2_13['avg_alleles_per_marker'] = grouped_by_sample_eff.mean().reindex(df_features_v2_13.index)
        df_features_v2_13['markers_gt2_alleles'] = grouped_by_sample_eff.apply(lambda x: (x > 2).sum()).reindex(df_features_v2_13.index)
        df_features_v2_13['markers_gt3_alleles'] = grouped_by_sample_eff.apply(lambda x: (x > 3).sum()).reindex(df_features_v2_13.index)
        df_features_v2_13['markers_gt4_alleles'] = grouped_by_sample_eff.apply(lambda x: (x > 4).sum()).reindex(df_features_v2_13.index)
        
        if 'Height' in df_effective_alleles.columns:
            grouped_heights_eff = df_effective_alleles.groupby('Sample File')['Height']
            df_features_v2_13['avg_peak_height'] = grouped_heights_eff.mean().reindex(df_features_v2_13.index)
            df_features_v2_13['std_peak_height'] = grouped_heights_eff.std().reindex(df_features_v2_13.index)
        else:
            print("警告: df_effective_alleles 缺少 'Height' 列，峰高相关特征无法计算。")
    
    df_features_v2_13.fillna(0, inplace=True)
    df_features_v2_13.reset_index(inplace=True)

    print("\n--- 特征工程完成 ---")
    print(f"最终特征数据框 df_features_v2_13 维度: {df_features_v2_13.shape}")
    print_df_in_chinese(df_features_v2_13.head(), col_map=COLUMN_TRANSLATION_MAP, title="新特征数据框 (df_features_v2_13) 前5行")
    
    try:
        df_features_v2_13.to_csv(feature_filename_prob1, index=False, encoding='utf-8-sig')
        print(f"特征数据已保存到: {feature_filename_prob1}")
    except Exception as e:
        print(f"错误: 保存特征数据失败: {e}")

except Exception as e_feat_eng_v2_13:
    print(f"严重错误: 在特征工程阶段发生错误: {e_feat_eng_v2_13}")
    import traceback
    traceback.print_exc();
print("--- 步骤 3 完成 ---")
...
# --- 步骤 4 & 5: 模型评估 (框架) ---
print(f"\n--- 步骤 4 & 5: 模型评估 (版本 2.13) ---")
# <<<<< 在此插入或确保你的模型训练和评估代码, 输入为 df_features_v2_13 >>>>>
print("--- 步骤 4 & 5 (框架) 完成 ---")

print(f"\n脚本 {os.path.basename(__file__)} (版本 2.13) 执行完毕。")
