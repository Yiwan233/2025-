...
代码名称: P1_V2.6_Final_Stutter_Workflow
版本: 2.6
日期: 2025-05-12
描述: 最终整合版，从JSON加载参数，修正了Stutter评估函数中的UnboundLocalError，
      实现了完整的数据加载、Stutter概率化评估、特征工程的工作流。
      输出包含真实等位基因置信度(CTA)的处理后峰数据(df_processed_peaks)
      和基于此计算的新特征集(df_features_v2_6)。
"""
# --- 基础库与机器学习库导入 ---
import pandas as pd
import re
...
PLOTS_DIR = os.path.join(DATA_DIR, 'prob1_plots_v2.6_final_eval')
...
print(f"\n--- 从JSON文件加载全局参数 (版本 2.6) ---")
config = {}; SATURATION_THRESHOLD = 30000.0; SIZE_TOLERANCE_BP = 0.5; STUTTER_CV_HS_GLOBAL = 0.25; 
...
# --- 核心：峰处理与Stutter评估函数 (版本 2.6 - 修正UnboundLocalError) ---
...
def calculate_peak_confidence_v2_6(locus_peaks_df_input, marker_name, marker_params_dict,
    ...
    # 使用 .reset_index(drop=True) 避免不必要的 'index' 列，或者如果需要原始顺序的映射，则在candidate_peaks_list中保留原始索引
    ...
    if l_repeat == 0 or \
       ...
       stutter_params_n_minus_1.get('RS_max_k', 0.0) <= 1e-9: # 如果RS_max_k为0或极小，则不评估
        peaks_df['CTA'] = 1.0; peaks_df['Is_Stutter_Suspect'] = False
        # Stutter_Score_N_Minus_1 已初始化为0.0
    ...
    # temp_max_stutter_scores_for_each_peak 列表用于存储每个候选峰作为stutter的最终最大分数
    # 它的大小应该与 peaks_df 的行数一致，并在循环外初始化
    ...
    for i in range(len(peaks_df)): # pc_row is potential stutter
        ...
        # *** 核心修正: 初始化用于累积当前 pc_row (i) 的最大stutter分数的变量 ***
        max_score_this_pc_is_stutter_from_any_parent = 0.0 
        
        for j in range(len(peaks_df)): # pp_row is potential parent
            if i == j: continue # 一个峰不能是自己的stutter
            pp_row = peaks_df.iloc[j]
            
            # Stutter峰通常严格小于其亲代峰高 (这里使用 * 1.01 是为了避免浮点数精度问题导致相等也被跳过)
            # 更严格的可以是 pc_row['Height'] < pp_row['Height']
            if pc_row['Height'] >= pp_row['Height']: # 简化：stutter峰高必须小于亲代峰高
                 continue

            is_pos_match_n_minus_1 = (abs((pp_row['Size'] - pc_row['Size']) - l_repeat) <= size_tolerance)
            
            score_stutter_for_this_parent_pair = 0.0 # 初始化这对峰的stutter分数
            debug_entry = {} # 初始化调试日志条目为空字典

            if is_pos_match_n_minus_1:
                # 仅当位置匹配时，才收集详细的调试信息
                if debug_log_list_param is not None:
                    debug_entry = {
                        'Marker': marker_name, 'Parent_Allele_Raw': pp_row['Allele'], 
                        'Parent_Allele_Numeric': pp_row['Allele_Numeric'], 'Parent_Size': pp_row['Size'], 
                        ...
                        'ESR_Model_Type_Used': stutter_params_n_minus_1.get('SR_model_type', 'N/A'), # 记录使用的模型类型
                        'ESR_m_Used': stutter_params_n_minus_1.get('SR_m', np.nan), # 记录使用的m
                        'ESR_c_Used': stutter_params_n_minus_1.get('SR_c', np.nan), # 记录使用的c
                        'Expected_SR': np.nan, 'Expected_Stutter_Height': np.nan, 
                        'Assumed_CV_Hs': cv_hs_to_use, 'Calculated_Sigma_Hs': np.nan, 
                        'Z_Score': np.nan, 'Calculated_Stutter_Score_for_this_Pair': 0.0
                    }
                ...
                if sr_obs > sr_max_n_minus_1: 
                    ...
                    if pd.notna(parent_an) and parent_an >= 0: # 确保亲代等位基因是有效的数字用于模型
                        model_type = stutter_params_n_minus_1.get('SR_model_type')
                        m_val = stutter_params_n_minus_1.get('SR_m', np.nan) # 从参数获取m
                        c_val = stutter_params_n_minus_1.get('SR_c', np.nan) # 从参数获取c
                    ...
                    if pd.notna(e_sr): e_sr = max(0.001, e_sr) # 避免E[SR]为0或负
                    ...
                    calculated_score_for_pair = 0.0 # 重命名以避免与外部变量冲突
                    if pd.notna(e_hs) and e_hs > 1e-6 :
                        sigma_hs = cv_hs_to_use * e_hs
                        if debug_entry: debug_entry['Calculated_Sigma_Hs'] = sigma_hs
                        if sigma_hs > 1e-6: # 避免sigma_hs为0导致除零错误
                            z_score = (pc_row['Height'] - e_hs) / sigma_hs
                            if debug_entry: debug_entry['Z_Score'] = z_score
                            calculated_score_for_pair = exp(-0.5 * (z_score**2))
                            # 可选的惩罚
                            if pd.notna(e_sr) and e_sr > 1e-6: # 仅当e_sr有效时应用惩罚
                                if sr_obs > e_sr * 1.8 : calculated_score_for_pair *= 0.5 
                                if sr_obs > e_sr * 2.5 : calculated_score_for_pair *= 0.1
                        else: # sigma_hs 极小 (E[Hs]很小或CV_Hs为0)，要求严格匹配
                            calculated_score_for_pair = 1.0 if abs(pc_row['Height'] - e_hs) < (0.001 * e_hs + 1e-6) else 0.0
                    elif pd.notna(e_hs) and pc_row['Height'] < 1e-6 : # E[Hs]为0，则只有当观察到的Hc也为0时才完全匹配
                        calculated_score_for_pair = 1.0
                    
                    score_stutter_for_this_parent_pair = calculated_score_for_pair
                
                if debug_entry: 
                    debug_entry['Calculated_Stutter_Score_for_this_Pair'] = score_stutter_for_this_parent_pair
                    if debug_log_list_param is not None: # 确保列表已传递
...
for sample_file_name, group_data_per_sample in df_prob1.groupby('Sample File'):
    ...
    for marker_name_actual, locus_data_from_groupby in group_data_per_sample.groupby('Marker'):
        ...
        processed_locus_df = calculate_peak_confidence_v2_6( 
...
print("\n所有样本位点处理完成。")
if not processed_peak_data_all_samples: print("警告: 处理所有样本后，列表 processed_peak_data_all_samples 为空。"); df_processed_peaks = pd.DataFrame()
...
else: print("df_processed_peaks 为空。")
...
try:
    ...
    if not df_processed_peaks.empty:
        ...
        df_effective_alleles = pd.DataFrame(columns=df_processed_peaks.columns.tolist() + ['Is_Effective_Allele'])
    ...
    if not df_effective_alleles.empty:
        ...
        for col, series in {
            'max_allele_per_sample': grouped_by_sample_eff.max(),'total_alleles_per_sample': grouped_by_sample_eff.sum(),
            'avg_alleles_per_marker': grouped_by_sample_eff.mean(),'markers_gt2_alleles': grouped_by_sample_eff.apply(lambda x: (x > 2).sum()),
            'markers_gt3_alleles': grouped_by_sample_eff.apply(lambda x: (x > 3).sum()),'markers_gt4_alleles': grouped_by_sample_eff.apply(lambda x: (x > 4).sum())
        ...
        for col in ['max_allele_per_sample', 'total_alleles_per_sample', 'avg_alleles_per_marker', 'markers_gt2_alleles', 'markers_gt3_alleles', 'markers_gt4_alleles','avg_peak_height', 'std_peak_height']:
...
# --- 步骤 4 & 5: 模型评估 ---
# <<<<< 您需要在此处填入您之前版本中完整且能成功运行的Step 4 和 Step 5代码, 输入为 df_features_v2_6 >>>>>
print(f"\n--- 步骤 4 & 5: 模型评估 (版本 2.6) ---")
print("--- 步骤 4 & 5 (框架) 完成 ---")
