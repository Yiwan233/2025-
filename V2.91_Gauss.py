...
try:
    ...
    # 交叉验证评估
    print("\n正在进行高斯过程分类器交叉验证 (可能需要较长时间)...")
    gp_cv_scores = cross_val_score(gp_model, X_scaled, y, cv=cv, scoring='accuracy')
    print(f"高斯过程分类器交叉验证准确率: {gp_cv_scores.mean():.4f} (±{gp_cv_scores.std():.4f})")
    ...
    print(f"高斯过程分类器测试集准确率: {gp_accuracy:.4f}")
    
    # 输出分类报告
    class_names = [f"{i}人" for i in sorted(y.unique())]
    print("\n高斯过程分类器分类报告:")
    print(classification_report(y_test, y_pred_gp, target_names=class_names))
    
    # 保存最佳模型
    best_model = gp_model
    best_model_name = "高斯过程分类器"
    y_pred = y_pred_gp
    best_model_acc = gp_accuracy
    ...
    confusion_matrix_path = os.path.join(PLOTS_DIR, 'confusion_matrix_gp_v2.91.png')
    ...
    try:
        ...
        plt.tight_layout()
        proba_plot_path = os.path.join(PLOTS_DIR, 'gp_probabilities_v2.91.png')
    ...
    # 保存模型
    model_filename = os.path.join(DATA_DIR, f'noc_gaussian_process_v2.91.joblib')
    ...
    print(f"高斯过程模型训练过程中发生错误: {e_model}")
    import traceback
    traceback.print_exc()
    # 确保即使模型训练失败，仍能继续执行
...
try:
    ...
    # 检查是否有训练好的高斯过程模型
    if 'gp_model' in locals() and 'X_test' in locals():
        print("正在计算高斯过程模型的SHAP值...")
        
        # 由于高斯过程模型结构复杂，使用KernelExplainer
        # 注意：KernelExplainer对计算资源要求较高，使用更小的样本数量
        sample_size = min(50, len(X_test))
        X_sample = X_test.iloc[:sample_size].copy()
        background = shap.kmeans(X_train, 10)  # 使用K-means生成背景数据集
        
        # 创建一个包括预处理的pipeline
        print("创建可解释的模型管道...")
        explainer = shap.KernelExplainer(
            model=gp_model.predict_proba, 
            data=background,
            link="logit"
        )
        
        # 对样本计算SHAP值 (指定输出类别0对应的概率)
        print("计算样本的SHAP值 (这可能需要一些时间)...")
        # 我们选择解释预测为第一个类别的概率
        class_to_explain = 0  # 可以根据需要更改为其他类别
        ...
        if is_multiclass:
            print(f"多分类问题，共{len(shap_values)}个类别")
        ...
        plt.figure(figsize=(12, 10))
        if is_multiclass:
            # 选择一个类别进行可视化 (例如，第一个类别)
            plt.title(f"SHAP值: 预测为类别{class_to_explain}的解释")
            shap.summary_plot(shap_values[class_to_explain], X_sample, show=False)
        else:
            plt.title("SHAP值: 总体特征重要性")
            shap.summary_plot(shap_values, X_sample, show=False)
        
        shap_summary_path = os.path.join(PLOTS_DIR, 'gp_shap_summary_v2.91.png')
        plt.savefig(shap_summary_path)
        plt.close()
        print(f"SHAP总结图已保存至: {shap_summary_path}")
        
        # 为多个类别创建SHAP条形图
        if is_multiclass and num_classes > 1:
            for class_idx in range(min(3, num_classes)):  # 最多显示前3个类别
                plt.figure(figsize=(12, 8))
                plt.title(f"SHAP特征重要性 (类别 {class_idx})")
                shap.summary_plot(shap_values[class_idx], X_sample, plot_type="bar", show=False)
                class_shap_path = os.path.join(PLOTS_DIR, f'gp_shap_class_{class_idx}_v2.91.png')
                plt.savefig(class_shap_path)
                plt.close()
                print(f"类别 {class_idx} 的SHAP图已保存至: {class_shap_path}")
        
        # 创建SHAP依赖图 - 选择前3个最重要的特征进行可视化
        try:
            # 计算每个特征的平均绝对SHAP值来确定特征重要性
            feature_importance = {}
            for feature_idx in range(X_sample.shape[1]):
                if is_multiclass:
                    # 对所有类别的SHAP值求平均
                    importance_val = np.mean([np.abs(shap_values[c][:, feature_idx]).mean() 
                                             for c in range(num_classes)])
                else:
                    importance_val = np.abs(shap_values[:, feature_idx]).mean()
                feature_importance[X_sample.columns[feature_idx]] = importance_val
            
            # 排序并选择前3个特征
            sorted_features = sorted(feature_importance.items(), key=lambda x: x[1], reverse=True)
            top_features = [f[0] for f in sorted_features[:3]]
            
            print(f"前3个最重要的特征: {top_features}")
            ...
            for feature in top_features:
                ...
                if is_multiclass:
                    ...
                    plt.title(f"SHAP依赖图: {feature} (类别 {class_to_explain})")
                ...
                dependency_path = os.path.join(PLOTS_DIR, f'gp_shap_dependency_{feature}_v2.91.png')
                plt.savefig(dependency_path)
                plt.close()
                print(f"特征 {feature} 的依赖图已保存至: {dependency_path}")
                
        except Exception as e_dep:
            print(f"创建SHAP依赖图时发生错误: {e_dep}")
        
        # 创建SHAP力图 (waterfall plot) - 选择一个样本进行详细解释
        try:
            # 选择一个样本进行解释
            sample_to_explain = 0  # 第一个样本
            
            # 使用决策图代替水瀑图，因为水瀑图在多分类问题上可能会有维度问题
            plt.figure(figsize=(12, 8))
            plt.title(f"样本 {sample_to_explain} 的SHAP决策图")
            
            # 检查shap_values的结构并正确提取值
            if is_multiclass:
                # 对于多分类问题
                sample_shap_values = shap_values[class_to_explain][sample_to_explain]
                
                # 检查是否为数组的数组
                if isinstance(sample_shap_values[0], (list, np.ndarray)):
                    print("检测到嵌套数组，使用平均值")
                    # 如果是嵌套数组，取平均值或第一个元素
                    sample_shap_values = np.array([np.mean(val) if isinstance(val, (list, np.ndarray)) else val 
                                                for val in sample_shap_values])
            else:
                # 二分类情况
                sample_shap_values = shap_values[sample_to_explain]
                if isinstance(sample_shap_values[0], (list, np.ndarray)):
                    print("检测到嵌套数组，使用平均值")
                    sample_shap_values = np.array([np.mean(val) if isinstance(val, (list, np.ndarray)) else val 
                                                for val in sample_shap_values])
            ...
            # 打印SHAP值的形状以便调试
            print(f"SHAP值形状: {np.array(sample_shap_values).shape}")
            print(f"SHAP值类型: {type(sample_shap_values)}")
            if len(sample_shap_values) > 0:
                print(f"第一个SHAP值类型: {type(sample_shap_values[0])}")
            ...
            if len(sample_shap_values) != len(feature_names):
                print(f"警告: SHAP值数量 ({len(sample_shap_values)}) 与特征数量 ({len(feature_names)}) 不匹配")
                # 截断或扩展以匹配维度
                if len(sample_shap_values) > len(feature_names):
                    sample_shap_values = sample_shap_values[:len(feature_names)]
                else:
                    # 用0填充缺失的值
                    sample_shap_values = np.pad(sample_shap_values, 
                                           (0, len(feature_names) - len(sample_shap_values)),
                                           'constant')
            
            # 按绝对值大小排序
            indices = np.argsort(np.abs(sample_shap_values))
            indices = indices[-15:]  # 显示最重要的15个特征
            
            # 创建条形图时确保颜色选择是基于标量比较
            colors = []
            for val in sample_shap_values[indices]:
                # 确保val是标量
                scalar_val = float(val) if hasattr(val, "__len__") else val
                colors.append('red' if scalar_val > 0 else 'blue')
            
            plt.barh(
                y=np.array(feature_names)[indices],
                width=sample_shap_values[indices],
                color=colors
            )
            plt.axvline(x=0, color='black', linestyle='-', alpha=0.3)
            plt.xlabel('SHAP值 (对样本预测的影响)')
            plt.tight_layout()
            
            # 保存图片
            force_plot_path = os.path.join(PLOTS_DIR, f'gp_shap_decision_sample_{sample_to_explain}_v2.91.png')
            plt.savefig(force_plot_path)
            plt.close()
            print(f"样本 {sample_to_explain} 的SHAP决策图已保存至: {force_plot_path}")
            
            # 尝试使用另一种方式创建力图 - 包含特征值
            try:
                plt.figure(figsize=(16, 8))
                plt.title(f"样本 {sample_to_explain} 的SHAP力量图")
                
                # 获取样本的特征值
                features = X_sample.iloc[sample_to_explain].values
                
                # 创建DataFrame以便排序和显示
                df_plot = pd.DataFrame({
                    'SHAP值': sample_shap_values,
                    '特征值': features,
                    '特征名': feature_names
                })
                
                # 确保SHAP值是标量
                df_plot['SHAP值'] = df_plot['SHAP值'].apply(lambda x: float(x) if hasattr(x, "__len__") else x)
                
                # 计算绝对值并排序
                df_plot['abs_shap'] = np.abs(df_plot['SHAP值'])
                df_plot = df_plot.sort_values('abs_shap', ascending=False).head(15)
                
                # 创建颜色列表
                bar_colors = ['red' if x > 0 else 'blue' for x in df_plot['SHAP值']]
                ...
                # 添加特征名称和特征值
                labels = [f"{name} = {value:.3f}" for name, value in zip(df_plot['特征名'], df_plot['特征值'])]
                ...
                plt.tight_layout()
                enhanced_plot_path = os.path.join(PLOTS_DIR, f'gp_shap_enhanced_sample_{sample_to_explain}_v2.91.png')
                plt.savefig(enhanced_plot_path)
                plt.close()
                print(f"样本 {sample_to_explain} 的增强SHAP图已保存至: {enhanced_plot_path}")
            except Exception as e_enhanced:
                print(f"创建增强SHAP图时发生错误: {e_enhanced}")
                import traceback
                traceback.print_exc()
        ...
        try:
            ...
            if is_multiclass:
                ...
                for feature_idx, feature_name in enumerate(X_sample.columns):
                    importance = np.mean([np.abs(shap_values[c][:, feature_idx]).mean() 
                                         for c in range(num_classes)])
                    feature_importances[feature_name] = float(importance)
            else:
                feature_importances = {}
                for feature_idx, feature_name in enumerate(X_sample.columns):
                    importance = np.abs(shap_values[:, feature_idx]).mean()
                    feature_importances[feature_name] = float(importance)
            ...
            shap_analysis = {
                ...
                "top_features": [{"name": name, "importance": float(imp)} for name, imp in sorted_importances[:10]],
                "classes_analyzed": num_classes,
            ...
            # 保存为JSON
            import json
            shap_analysis_file = os.path.join(DATA_DIR, 'gp_shap_analysis_v2.91.json')
        ...
        print("未找到高斯过程模型或测试数据，无法计算SHAP值")
