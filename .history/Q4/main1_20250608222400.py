...
class UPG_M_Pipeline:
    ...
    def prepare_observed_data(self, sample_peaks: pd.DataFrame) -> Dict:
        ...
        logger.info(f"准备完成 {len(observed_data)} 个位点的观测数据")
...
def analyze_single_sample_upg(sample_id: str, att4_path: str = None) -> Dict:
    ...
    # 加载数据
    sample_groups = pipeline.load_attachment4_data()
    
    if sample_id not in sample_groups:
        logger.error(f"样本 {sample_id} 未在附件4中找到")
        available_samples = list(sample_groups.keys())[:5]  # 显示前5个可用样本
        logger.info(f"可用样本示例: {available_samples}")
        return {}
    
    # 分析指定样本
    sample_data = sample_groups[sample_id]
    result = pipeline.analyze_sample(sample_id, sample_data)
    
    # 生成可视化
    if result.get('mcmc_results'):
        pipeline.visualizer.plot_denoising_results(
            sample_id, result['observed_data'], 
            result['denoised_results'], result['mcmc_results']
        )
        
        pipeline.visualizer.plot_comparison_original_vs_denoised(
            sample_id, result['observed_data'], result['denoised_results']
        )
    
    # 保存结果
    output_file = os.path.join(config.OUTPUT_DIR, f'{sample_id}_upg_analysis.json')
    pipeline.save_results(result, output_file)
    
    logger.info(f"样本 {sample_id} 的UPG-M分析完成，结果已保存")
    return result

def analyze_all_samples_upg(att4_path: str = None, max_samples: int = None) -> Dict:
    """使用UPG-M方法分析附件4中的所有样本"""
    if att4_path is None:
        att4_path = config.ATTACHMENT4_PATH
    ...
    logger.info(f"开始分析 {len(sample_ids)} 个样本")
    ...
    performance_summary = {
        ...
        'noc_accuracy_scores': [],
        'mixing_ratio_accuracies': [],
        'noise_reduction_rates': []
    }
    
    for i, sample_id in enumerate(sample_ids, 1):
        try:
            logger.info(f"分析进度: {i}/{len(sample_ids)} - {sample_id}")
            ...
            if result.get('mcmc_results'):
                ...
                performance_summary['successful_analyses'] += 1
                
                # 收集性能指标
                metrics = result.get('performance_metrics', {})
                if 'noc_accuracy' in metrics:
                    performance_summary['noc_accuracy_scores'].append(metrics['noc_accuracy'])
                if 'mixing_ratio_accuracy' in metrics:
                    performance_summary['mixing_ratio_accuracies'].append(metrics['mixing_ratio_accuracy'])
                if 'noise_reduction_rate' in metrics:
                    performance_summary['noise_reduction_rates'].append(metrics['noise_reduction_rate'])
                
                performance_summary['average_computation_time'] += result.get('computation_time', 0.0)
            
            else:
                logger.warning(f"样本 {sample_id} 分析失败")
                performance_summary['failed_analyses'] += 1
        
        except Exception as e:
            logger.error(f"样本 {sample_id} 分析异常: {e}")
            performance_summary['failed_analyses'] += 1
    
    # 计算总体性能指标
    if performance_summary['successful_analyses'] > 0:
        performance_summary['average_computation_time'] /= performance_summary['successful_analyses']
        
        if performance_summary['noc_accuracy_scores']:
            performance_summary['overall_noc_accuracy'] = np.mean(performance_summary['noc_accuracy_scores'])
        
        if performance_summary['mixing_ratio_accuracies']:
            performance_summary['overall_mixing_accuracy'] = np.mean(performance_summary['mixing_ratio_accuracies'])
        
        if performance_summary['noise_reduction_rates']:
            performance_summary['overall_noise_reduction'] = np.mean(performance_summary['noise_reduction_rates'])
    
    # 保存总体结果
    summary_file = os.path.join(config.OUTPUT_DIR, 'upg_batch_analysis_summary.json')
    with open(summary_file, 'w', encoding='utf-8') as f:
        json.dump(performance_summary, f, ensure_ascii=False, indent=2)
    
    logger.info(f"批量分析完成：成功 {performance_summary['successful_analyses']} 个，"
               f"失败 {performance_summary['failed_analyses']} 个")
...
def generate_comprehensive_report(analysis_results: Dict, output_file: str = None):
    """生成综合分析报告（文本格式）"""
    if output_file is None:
        output_file = os.path.join(config.OUTPUT_DIR, 'UPG_M_Comprehensive_Report.txt')
    ...
    report_lines = [
        "=" * 80,
        "🧬 UPG-M法医STR混合样本降噪系统综合分析报告",
        "Unified Probabilistic Genotyping with MCMC-based Noise Reduction",
        "=" * 80,
        f"报告生成时间: {time.strftime('%Y年%m月%d日 %H:%M:%S')}",
        "",
        "📊 分析概览",
        "-" * 40,
        f"总样本数:           {perf_summary.get('total_samples', 0)}",
        f"成功分析:           {perf_summary.get('successful_analyses', 0)}",
        f"分析失败:           {perf_summary.get('failed_analyses', 0)}",
        f"平均计算时间:       {perf_summary.get('average_computation_time', 0):.1f}秒",
        "",
        "🎯 性能指标",
        "-" * 40,
        f"NoC识别准确率:      {perf_summary.get('overall_noc_accuracy', 0):.3f}",
        f"混合比例准确率:     {perf_summary.get('overall_mixing_accuracy', 0):.3f}",
        f"平均降噪率:         {perf_summary.get('overall_noise_reduction', 0):.3f}",
        "",
        "📋 详细分析结果",
        "-" * 40,
    ]
    ...
    if 'results' in analysis_results:
        report_lines.extend([
            f"{'样本ID':<30} {'推断NoC':<8} {'置信度':<8} {'主要比例':<10} {'降噪峰':<8} {'计算时间':<10} {'MCMC收敛':<10}",
            "-" * 90,
        ])
        
        for sample_id, result in analysis_results['results'].items():
            denoised = result.get('denoised_results', {})
            consensus_noc = denoised.get('consensus_noc', {})
            mixing_ratios = denoised.get('denoised_mixture_ratios', {})
            noise_class = denoised.get('noise_classification', {})
            mcmc_results = result.get('mcmc_results', {})
            
            map_noc = consensus_noc.get('map_noc', 'N/A')
            confidence = consensus_noc.get('confidence', 0)
            main_ratio = mixing_ratios.get('map_ratios', [0])[0] if mixing_ratios.get('map_ratios') else 0
            noise_peaks = len(noise_class.get('noise_peaks', {}))
            comp_time = result.get('computation_time', 0)
            converged = mcmc_results.get('converged', False)
            
            convergence_text = '✓收敛' if converged else '⚠需检查'
            
            report_lines.append(
                f"{sample_id:<30} {map_noc:<8} {confidence:<8.3f} {main_ratio:<10.3f} "
                f"{noise_peaks:<8} {comp_time:<10.1f} {convergence_text:<10}"
            )
    ...
    report_lines.extend([
        ...
        "🔧 UPG-M技术特点",
        "-" * 40,
        "• 整体性推断：同时推断NoC、混合比例、基因型和噪声参数",
        "• Trans-dimensional MCMC：动态调整贡献者人数空间",
        "• V5特征驱动：基于V5特征的自适应参数估计",
        "• 多层噪声建模：Stutter、ADO、基线噪声的综合处理",
        "• 后验降噪：基于贝叶斯后验的智能峰分类",
        "• 置信度量化：完整的不确定性量化和置信区间",
        "",
        "📈 算法优势",
        "-" * 40,
        "",
    ...
    # 添加优势总结
    success_rate = perf_summary.get('successful_analyses', 0) / max(perf_summary.get('total_samples', 1), 1)
    noc_accuracy = perf_summary.get('overall_noc_accuracy', 0)
    noise_reduction = perf_summary.get('overall_noise_reduction', 0)
    
    report_lines.extend([
        f"• 分析成功率:       {success_rate:.1%}",
        f"• NoC识别准确率:    {noc_accuracy:.1%}",
        f"• 平均降噪效果:     {noise_reduction:.1%}",
        f"• 平均处理时间:     {perf_summary.get('average_computation_time', 0):.1f}秒/样本",
        "",
        "📝 结论",
        "-" * 40,
        "UPG-M系统成功实现了STR混合样本的统一概率基因分型和智能降噪，",
        "通过Trans-dimensional MCMC和V5特征驱动的参数估计，显著提高了",
        "混合样本分析的准确性和鲁棒性。系统能够自动推断贡献者人数、",
        "混合比例，并有效去除各种类型的噪声，为法医DNA分析提供了",
        "先进的技术解决方案。",
        "",
        "=" * 80,
        "报告结束",
    ...
    logger.info(f"综合报告已保存至: {output_file}")

def main():
    """主函数"""
    print("🧬 UPG-M法医STR混合样本降噪系统启动")
    print("=" * 80)
    
    # 检查数据文件
    if not os.path.exists(config.ATTACHMENT4_PATH):
        print(f"❌ 错误：找不到附件4数据文件 {config.ATTACHMENT4_PATH}")
        return
    ...
    try:
        ...
        if choice == "1":
            ...
            if sample_groups:
                sample_ids = list(sample_groups.keys())
                print(f"\n可用样本数: {len(sample_ids)}")
                print("前5个样本:", sample_ids[:5])
                
                sample_id = input("请输入要分析的样本ID: ").strip()
                if sample_id:
                    result = analyze_single_sample_upg(sample_id)
                    if result:
                        ...
                        denoised = result.get('denoised_results', {})
                        if denoised:
                            noc_info = denoised.get('consensus_noc', {})
                            mixing_info = denoised.get('denoised_mixture_ratios', {})
                            noise_info = denoised.get('noise_classification', {})
                            
                            print(f"📊 推断结果：")
                            print(f"   • NoC: {noc_info.get('map_noc', 'N/A')}")
                            print(f"   • 置信度: {noc_info.get('confidence', 0):.3f}")
                            
                            map_ratios = mixing_info.get('map_ratios', [])
                            if map_ratios:
                                print(f"   • 混合比例: {[f'{r:.3f}' for r in map_ratios]}")
                            
                            print(f"   • 真实等位基因: {len(noise_info.get('true_alleles', {}))}")
                            print(f"   • Stutter伪影: {len(noise_info.get('stutter_artifacts', {}))}")
                            print(f"   • 噪声峰: {len(noise_info.get('noise_peaks', {}))}")
                    else:
                        print(f"❌ 样本 {sample_id} 分析失败")
        
        elif choice == "2":
            # 完整分析
            print("\n🚀 开始完整分析...")
            results = analyze_all_samples_upg()
            ...
            # 批量测试
            try:
                n_samples = int(input("请输入要分析的样本数量: ").strip())
                print(f"\n🧪 开始分析前{n_samples}个样本...")
                
                results = analyze_all_samples_upg(max_samples=n_samples)
