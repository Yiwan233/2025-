...
from q4_enhanced_visualization import Q4EnhancedVisualization
    ...
    def extract_mixture_ratios_from_filename(self, filename):
        ...
        if match:
            ...
            try:
                ...
                if len(contributor_ids) == len(ratios):
                    # 归一化比例
                    total = sum(ratios)
                    if total > 0:
                        normalized_ratios = [r/total for r in ratios]
                        logger.info(f"从文件名提取混合比例: {contributor_ids} -> {normalized_ratios}")
                        return contributor_ids, normalized_ratios
                    else:
                        logger.warning(f"混合比例总和为0: {filename}")
                        return contributor_ids, [1.0/len(contributor_ids)] * len(contributor_ids)
                else:
                    logger.warning(f"贡献者数({len(contributor_ids)})与比例数({len(ratios)})不匹配: {filename}")
                    return contributor_ids, None
            except ValueError as e:
                logger.warning(f"无法解析混合比例 '{ratio_str}': {e}")
                return contributor_ids, None
        
        logger.debug(f"无法从文件名提取混合比例: {filename}")
        return None, None
...
class IntelligentPeakClassifier:
    ...
    def extract_peak_features(self, peak_data, locus_context):
        ...
        # 基础峰特征 - 添加安全检查
        features['height'] = float(peak_data.get('Height', 0))
        features['size'] = float(peak_data.get('Size', 200))  # 默认STR大小
        features['relative_intensity'] = float(peak_data.get('Relative_Intensity', 0))
        features['height_rank'] = int(peak_data.get('Height_Rank', 1))
        features['is_saturated'] = int(peak_data.get('Is_Saturated', False))
        
        # 上下文特征（位点层面） - 添加安全检查
        if not locus_context.empty:
            locus_peaks = locus_context
            features['locus_peak_count'] = len(locus_peaks)
            
            heights = locus_peaks['Height'].values
            heights = heights[~np.isnan(heights)]  # 移除NaN值
            
            if len(heights) > 0:
                features['locus_max_height'] = float(np.max(heights))
                features['locus_mean_height'] = float(np.mean(heights))
                if len(heights) > 1 and np.mean(heights) > 0:
                    features['locus_height_cv'] = float(np.std(heights) / np.mean(heights))
                else:
                    features['locus_height_cv'] = 0.0
            else:
                features['locus_max_height'] = 0.0
                features['locus_mean_height'] = 0.0
                features['locus_height_cv'] = 0.0
        else:
            features['locus_peak_count'] = 1
            features['locus_max_height'] = features['height']
            features['locus_mean_height'] = features['height']
            features['locus_height_cv'] = 0.0
        
        # 相对位置特征 - 添加安全检查
        if not locus_context.empty and len(locus_context) > 1:
            all_sizes_in_locus = locus_context['Size'].dropna().values
            ...
            if len(all_sizes_in_locus) > 1:
                ...
                features['min_size_diff'] = float(min(size_diffs)) if size_diffs else 0.0
                features['is_size_outlier'] = int(features['min_size_diff'] > 20)  # 20bp阈值
            else:
                features['min_size_diff'] = 0.0
        ...
        current_size = features['size']
        current_height = features['height']
        
        if not locus_context.empty:
            for _, other_peak in locus_context.iterrows():
                other_size = other_peak.get('Size', 0)
                other_height = other_peak.get('Height', 0)
                
                # 检查有效性
                if pd.isna(other_size) or pd.isna(other_height):
                    continue
                    
                if other_size > current_size:
                    size_diff = other_size - current_size
                    if 3 <= size_diff <= 5:  # 典型的n-1 stutter
                        potential_parent_heights.append(float(other_height))
        
        if potential_parent_heights and current_height > 0:
            max_parent_height = max(potential_parent_heights)
            if max_parent_height > 0:
                features['stutter_ratio'] = float(current_height / max_parent_height)
            else:
                features['stutter_ratio'] = 0.0
        ...
        # 噪声模式特征 - 添加安全检查
        height_for_log = max(features['height'], 1.0)  # 避免log(0)
        features['height_log'] = float(np.log10(height_for_log))
        
        size_normalized = (features['size'] - 100) / 300  # 标准化片段大小
        features['size_normalized'] = float(np.clip(size_normalized, -1, 1))  # 限制范围
        
        # 信号质量评估 - 添加安全检查
        features['signal_quality_score'] = float(self._calculate_signal_quality(peak_data, locus_context))
        
        # 最终检查：确保所有特征都是有限数值
        for key, value in features.items():
            if pd.isna(value) or np.isinf(value):
                logger.warning(f"特征 {key} 包含异常值 {value}，设为默认值")
                if 'ratio' in key or 'quality' in key or 'cv' in key:
                    features[key] = 0.0
                elif 'count' in key or 'rank' in key:
                    features[key] = 1.0
                elif 'height' in key:
                    features[key] = 100.0
                elif 'size' in key:
    ...
    def _calculate_signal_quality(self, peak_data, locus_context):
        ...
        # 相对强度权重
        rel_intensity = peak_data.get('Relative_Intensity', 0)
        if pd.isna(rel_intensity):
            rel_intensity = 0
        ...
        # 饱和度惩罚
        is_saturated = peak_data.get('Is_Saturated', False)
        if is_saturated:
            score -= 0.1
    ...
    def train_classifier(self, training_features, training_labels):
        ...
        if total_nan > 0:
            logger.info(f"发现 {total_nan} 个NaN值，进行填充处理")
            for i, (feature_name, nan_count) in enumerate(zip(self.feature_names, nan_counts)):
                if nan_count > 0:
                    logger.info(f"  特征 '{feature_name}': {nan_count} 个NaN值")
        
        # 使用多种策略填充NaN值
        X_cleaned = self._handle_missing_values(X)
        
        # 验证没有NaN值
        if np.isnan(X_cleaned).any():
            logger.warning("仍存在NaN值，使用零填充")
        ...
        if np.isnan(X_scaled).any() or np.isinf(X_scaled).any():
            logger.warning("标准化后存在NaN或Inf值，进行后处理")
        ...
        try:
            ...
            logger.warning(f"分层划分失败: {e}，使用随机划分")
            X_train, X_test, y_train, y_test = train_test_split(
        ...
        # 选择合适的分类器
        try:
            # 优先使用支持NaN的HistGradientBoostingClassifier
            ...
            logger.info("使用HistGradientBoostingClassifier（支持NaN处理）")
        except ImportError:
            # 备选：使用随机森林
            logger.info("使用RandomForestClassifier作为备选")
            ...
            if np.isnan(col_data).any():
                if 'height' in feature_name.lower():
                    ...
                    if np.isnan(median_val):
                        median_val = 100.0  # 默认峰高
                    X_cleaned[np.isnan(col_data), i] = median_val
                    
                elif 'ratio' in feature_name.lower() or 'relative' in feature_name.lower():
                    # 比例特征用0.5填充
                    X_cleaned[np.isnan(col_data), i] = 0.5
                    
                elif 'count' in feature_name.lower() or 'num' in feature_name.lower():
                    # 计数特征用0填充
                    X_cleaned[np.isnan(col_data), i] = 0.0
                    
                elif 'size' in feature_name.lower():
                    # 片段大小用200填充（典型STR大小）
                    X_cleaned[np.isnan(col_data), i] = 200.0
                    
                elif 'rank' in feature_name.lower():
                    # 排名特征用1填充
                    X_cleaned[np.isnan(col_data), i] = 1.0
                    ...
                    # 其他特征用均值填充，如果均值为NaN则用0填充
                    mean_val = np.nanmean(col_data)
                    if np.isnan(mean_val):
                        mean_val = 0.0
                    X_cleaned[np.isnan(col_data), i] = mean_val
    ...
    def classify_peak(self, peak_data, locus_context):
        ...
        try:
            ...
            logger.warning(f"模型预测失败: {e}，使用启发式规则")
            return self._assign_heuristic_label(peak_data, locus_context), 0.5
...
class UPGMInferenceEngine:
    ...
    def calculate_likelihood(self, observed_data, N, mixture_ratios, genotype_sets, theta):
        ...
        # 确保混合比例归一化
        mixture_ratios = np.array(mixture_ratios)
        ...
        for locus, locus_data in observed_data.items():
            try:
                ...
                # 数值稳定性检查
                if np.isnan(locus_likelihood) or np.isinf(locus_likelihood):
                    locus_likelihood = -1e6
                
                total_log_likelihood += locus_likelihood
                
            except Exception as e:
                logger.debug(f"位点{locus}似然计算失败: {e}")
                total_log_likelihood += -1e6
        
        # 最终检查
        if np.isnan(total_log_likelihood) or np.isinf(total_log_likelihood):
            total_log_likelihood = -1e8
    ...
    def mcmc_sampler(self, observed_data, initial_params=None):
        ...
        if initial_params is None:
            ...
            if self.current_v5_features:
                sample_file = self.current_v5_features.get('Sample File', '')
                
                # 提取真实NoC
                true_noc = self.v5_extractor.extract_noc_from_filename(sample_file)
                if not pd.isna(true_noc) and 1 <= true_noc <= 5:
                    current_N = int(true_noc)
                    logger.info(f"使用文件名中的真实NoC初始化: N={current_N}")
                    
                    # 尝试提取真实混合比例
                    contributor_ids, mixture_ratios = self.v5_extractor.extract_mixture_ratios_from_filename(sample_file)
                    if mixture_ratios and len(mixture_ratios) == current_N:
                        current_Mx = np.array(mixture_ratios)
                        logger.info(f"使用文件名中的真实混合比例初始化: Mx={current_Mx}")
    ...
    def mcmc_sampler(self, observed_data, initial_params=None):
        ...
        if initial_params is None:
            ...
            if self.current_v5_features:
                ...
                if not pd.isna(true_noc) and 1 <= true_noc <= 5:
                    ...
                    if mixture_ratios and len(mixture_ratios) == current_N:
                        ...
                        # 使用均匀分布初始化
                        current_Mx = np.ones(current_N) / current_N
                        logger.info(f"使用均匀分布初始化混合比例: Mx={current_Mx}")
                else:
                    # 使用V5特征预测初始化
                    current_N = self._predict_noc_from_v5(self.current_v5_features)
                    current_Mx = np.ones(current_N) / current_N
                    logger.info(f"使用V5特征预测初始化: N={current_N}")
            else:
                # 默认初始化
                current_N = 2
                current_Mx = np.array([0.5, 0.5])
                logger.info("使用默认值初始化: N=2, Mx=[0.5, 0.5]")
            ...
            logger.info(f"使用提供的初始参数: N={current_N}")
        ...
        try:
            ...
            logger.info(f"初始状态: N={current_N}, 似然={current_log_likelihood:.2f}, 先验={current_log_prior:.2f}")
        except Exception as e:
            logger.error(f"初始似然计算失败: {e}")
            # 返回简化结果
            return {
                'samples': {'N': [current_N], 'Mx': [current_Mx], 'log_likelihood': [0], 'log_posterior': [0]},
                'acceptance_rate': 0.0,
                'trans_dimensional_acceptance_rate': 0.0,
                'n_samples': 1,
                'converged': False,
                'initial_N': current_N,
                'final_noc_distribution': {current_N: 1}
            }
        ...
        for iteration in range(self.n_iterations):
            ...
            # 多种更新策略
            update_strategies = ['Mx_major', 'Mx_minor', 'theta_update']
            strategy = np.random.choice(update_strategies, p=[0.6, 0.3, 0.1])
            
            try:
                if strategy == 'Mx_major':
                    # 主要混合比例更新
                    proposed_Mx = self._propose_mixture_ratios(current_Mx, step_size=0.03)
                elif strategy == 'Mx_minor':
                    # 小幅调整
                    proposed_Mx = self._propose_mixture_ratios(current_Mx, step_size=0.01)
                else:
                    # theta参数更新（保持Mx不变）
                    proposed_Mx = current_Mx.copy()
                ...
                # 检查似然是否有效
                if np.isnan(proposed_log_likelihood) or np.isinf(proposed_log_likelihood):
                    proposed_log_likelihood = current_log_likelihood - 1e6
                ...
                log_ratio = np.clip(log_ratio, -10, 10)  # 防止数值溢出
                ...
                # 记录接受历史
                self._acceptance_history.append(int(accepted))
                if len(self._acceptance_history) > 1000:
                    self._acceptance_history = self._acceptance_history[-1000:]
                    
            except Exception as e:
                logger.warning(f"MCMC迭代 {iteration} 失败: {e}")
                accepted = False
            
            # 存储样本（预热后）
            if iteration >= self.n_warmup and iteration % self.thinning == 0:
                # 确保Mx归一化后存储
                normalized_Mx = current_Mx / np.sum(current_Mx)
                samples['N'].append(current_N)
                samples['Mx'].append(normalized_Mx.copy())
                samples['log_likelihood'].append(current_log_likelihood)
                samples['log_posterior'].append(current_log_posterior)
        
        # 计算最终接受率
        final_acceptance_rate = n_accepted / max(self.n_iterations, 1)
        trans_dimensional_acceptance_rate = n_trans_dimensional_accepted / max(n_trans_dimensional_attempts, 1) if n_trans_dimensional_attempts > 0 else 0.0
        
        # 分析最终结果
        if samples['N']:
            final_noc_dist = pd.Series(samples['N']).value_counts()
            most_probable_noc = final_noc_dist.index[0]
            logger.info(f"最可能的NoC: {most_probable_noc} (概率: {final_noc_dist.iloc[0]/len(samples['N']):.3f})")
        ...
        return {
            ...
            'initial_N': current_N,
            'final_noc_distribution': pd.Series(samples['N']).value_counts().to_dict() if samples['N'] else {current_N: 1}
        }
    ...
    def _propose_mixture_ratios(self, current_Mx, step_size=0.05):
        ...
        try:
            ...
            # 自适应步长
            if hasattr(self, '_acceptance_history'):
                recent_acceptance = np.mean(self._acceptance_history[-100:]) if len(self._acceptance_history) > 10 else 0.5
                if recent_acceptance < 0.2:
                    step_size *= 0.8  # 减小步长
                elif recent_acceptance > 0.6:
                    step_size *= 1.2  # 增大步长
                step_size = np.clip(step_size, 0.001, 0.2)
            
            # 方法1: 对数空间提议 (70%概率)
            if np.random.random() < 0.7:
                log_Mx = np.log(current_Mx + 1e-10)
                log_proposed = log_Mx + np.random.normal(0, step_size, N)
                proposed_Mx = np.exp(log_proposed)
            else:
                # 方法2: Dirichlet提议 (30%概率)
                alpha = current_Mx / step_size + 0.1
                alpha = np.maximum(alpha, 0.1)
                proposed_Mx = np.random.dirichlet(alpha)
            
            # 确保归一化
            proposed_Mx = np.maximum(proposed_Mx, 1e-6)  # 避免零值
            proposed_Mx = proposed_Mx / np.sum(proposed_Mx)
            
            return proposed_Mx
            
        except Exception as e:
            logger.warning(f"混合比例提议失败: {e}")
            # 安全的备选方案
            noise = np.random.normal(0, 0.01, len(current_Mx))
            proposed_Mx = current_Mx + noise
            proposed_Mx = np.maximum(proposed_Mx, 1e-6)
            proposed_Mx = proposed_Mx / np.sum(proposed_Mx)
            return proposed_Mx
    ...
    def _calculate_prior(self, N, Mx, genotypes, theta):
        ...
        if self.current_v5_features:
            ...
            if sample_file:
                true_noc = self.v5_extractor.extract_noc_from_filename(sample_file)
                if not pd.isna(true_noc) and 1 <= true_noc <= 5:
                    # 如果预测的N与真实NoC一致，给予高先验概率
                    if N == int(true_noc):
                        log_prior += np.log(0.8)  # 80%概率给真实NoC
                        logger.debug(f"N={N}与真实NoC={true_noc}一致，给予高先验")
                    else:
                        log_prior += np.log(0.2 / (config.N_MAX - 1))  # 其他NoC分享剩余20%
                        logger.debug(f"N={N}与真实NoC={true_noc}不一致，给予低先验")
                else:
                    # 无法提取真实NoC，使用V5特征预测
    ...
    def _predict_noc_from_v5(self, v5_features):
        """基于V5特征预测NoC（集成文件名信息）"""
        # 优先使用文件名中的真实NoC
        sample_file = v5_features.get('Sample File', '')
        if sample_file:
            true_noc = self.v5_extractor.extract_noc_from_filename(sample_file)
            if not pd.isna(true_noc) and 1 <= true_noc <= 5:
                logger.info(f"从文件名提取到真实NoC: {true_noc}")
                return int(true_noc)
        
        # 如果无法从文件名提取，使用V5特征预测
        logger.info("无法从文件名提取NoC，使用V5特征预测")
        
        # 这里可以集成Q1训练的模型
        mac_profile = v5_features.get('mac_profile', 2)
        avg_alleles = v5_features.get('avg_alleles_per_locus', 2)
        total_alleles = v5_features.get('total_distinct_alleles', 4)
        
        # 改进的启发式规则
        if mac_profile <= 2 and avg_alleles <= 2.2 and total_alleles <= 8:
            predicted_noc = 1
        elif mac_profile <= 3 and avg_alleles <= 2.8 and total_alleles <= 12:
            predicted_noc = 2
        elif mac_profile <= 5 and avg_alleles <= 3.5 and total_alleles <= 18:
            predicted_noc = 3
        elif mac_profile <= 7 and avg_alleles <= 4.2 and total_alleles <= 25:
            predicted_noc = 4
        else:
            predicted_noc = 5
        
        logger.info(f"基于V5特征预测NoC: {predicted_noc} (MAC:{mac_profile}, AvgAlleles:{avg_alleles:.2f})")
        return predicted_noc
...
class UPGMPipeline:
    ...
    def analyze_sample(self, sample_data, enable_mcmc=True):
        ...
        if enable_mcmc and observed_data:
            try:
                logger.info(f"开始MCMC推断，观测数据包含{len(observed_data)}个位点")
                mcmc_results = self.inference_engine.mcmc_sampler(observed_data)
                if mcmc_results and mcmc_results['samples']:
                    posterior_summary = self._generate_posterior_summary(mcmc_results)
                    logger.info("MCMC推断成功完成")
                else:
                    logger.warning("MCMC返回空结果")
            except Exception as e:
                logger.error(f"MCMC推断失败: {e}")
                import traceback
                traceback.print_exc()
        else:
            if not observed_data:
                logger.warning("没有观测数据，跳过MCMC推断")
            else:
                logger.info("MCMC推断被禁用")
    ...
    def _generate_posterior_summary(self, mcmc_results):
        ...
        # 确保所有混合比例样本都是归一化的
        normalized_mx_samples = []
        for mx_sample in samples['Mx']:
            mx_array = np.array(mx_sample)
            normalized_mx = mx_array / np.sum(mx_array)
            normalized_mx_samples.append(normalized_mx)
        
        mx_array = np.array(normalized_mx_samples)
        N = mx_array.shape[1]
        ...
        for i in range(N):
            ...
            summary[f'Mx_{i+1}'] = {
                'mean': float(np.mean(component_samples)),
                'std': float(np.std(component_samples)),
                'median': float(np.median(component_samples)),
                'credible_interval_95': np.percentile(component_samples, [2.5, 97.5]).tolist()
            }
        
        # 验证归一化
        total_mean = sum(summary[f'Mx_{i+1}']['mean'] for i in range(N))
        logger.info(f"混合比例均值总和: {total_mean:.6f}")
        
        return {
            'mixture_ratio_summary': {f'NoC_{N}': summary},
            'most_probable_noc': N,
            'noc_confidence': 1.0,
            'model_quality': {
...
def main():
    ...
    # 集成Q4分析结果
    enhanced_viz.integrate_with_q4_results(q4_analysis_result, output_dir)
