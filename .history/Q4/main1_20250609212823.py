    ...
    def mcmc_sampler(self, observed_data, initial_params=None):
            ...
            if np.random.random() < config.TRANS_DIMENSIONAL_PROB and iteration > self.n_warmup // 2:
                ...
                if proposed_N is not None:
                    ...
                    if np.random.random() < accept_prob:
                        ...
                        if iteration % 5000 == 0:
                            logger.info(f"  跨维移动成功: N={current_N}, 新似然={current_log_likelihood:.2f}")
                    else:
                        accepted = False
                else:
                    accepted = False
        ...
        if samples['N']:
            ...
            if self.current_v5_features:
                sample_file = self.current_v5_features.get('Sample File', '')
                true_noc = self.v5_extractor.extract_noc_from_filename(sample_file)
                if not pd.isna(true_noc):
                    is_correct = (most_probable_noc == int(true_noc))
                    logger.info(f"NoC预测准确性: {'✅ 正确' if is_correct else '❌ 错误'} "
                              f"(真实: {int(true_noc)}, 预测: {most_probable_noc})")
        ...
        return {
            ...
            'initial_N': current_N,
            'final_noc_distribution': pd.Series(samples['N']).value_counts().to_dict() if samples['N'] else {}
        }# -*- coding: utf-8 -*-
"""
