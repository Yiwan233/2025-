...
class Q1FeatureEngineering:
    ...
    def extract_v5_features(self, sample_file, sample_peaks):
        ...
        if sample_peaks.empty:
            logger.warning(f"样本 {sample_file} 峰数据为空")
            return self._get_default_features(sample_file)
        
        # 数据完整性检查
        logger.info(f"样本 {sample_file} 数据检查:")
        logger.info(f"  总峰数: {len(sample_peaks)}")
        logger.info(f"  位点数: {sample_peaks['Marker'].nunique()}")
        logger.info(f"  峰高分布: min={sample_peaks['Height'].min():.1f}, max={sample_peaks['Height'].max():.1f}")
        
        # 按位点分组检查
        locus_groups = sample_peaks.groupby('Marker')
        alleles_per_locus = locus_groups['Allele'].nunique()
        
        logger.info(f"  每位点等位基因数分布: {alleles_per_locus.value_counts().to_dict()}")
        
        # 检查是否有足够的杂合位点用于PHR计算
        heterozygous_loci = (alleles_per_locus == 2).sum()
        logger.info(f"  杂合位点数(用于PHR): {heterozygous_loci}")
        
        if heterozygous_loci == 0:
            logger.warning("没有杂合位点，PHR相关特征将为0")
            logger.info(f"开始提取V5特征 - 样本: {sample_file}")
            logger.info(f"峰数据形状: {sample_peaks.shape if not sample_peaks.empty else '空'}")
            
        if sample_peaks.empty:
            logger.warning(f"样本 {sample_file} 峰数据为空，使用默认特征")
            return self._get_default_features(sample_file)
        ...
        if total_peaks > 0:
            ...
            # 添加调试信息
            logger.info(f"峰高统计 - 总峰数: {total_peaks}")
            logger.info(f"峰高范围: {np.min(all_heights):.1f} - {np.max(all_heights):.1f}")
            logger.info(f"平均峰高: {features['avg_peak_height']:.1f}")
            logger.info(f"峰高标准差: {features['std_peak_height']:.1f}")
            
            if total_peaks > 2:
                features['skewness_peak_height'] = stats.skew(all_heights)
                logger.info(f"峰高偏度: {features['skewness_peak_height']:.3f}")
            else:
                features['skewness_peak_height'] = 0
                logger.warning("峰数不足，偏度设为0")
