...
class MGM_RF_Inferencer:
    ...
    def propose_mixture_ratios(self, current_ratios: np.ndarray, 
        ...
        """使用更激进的提议分布来改善探索"""
        # 大幅增加步长
        effective_step = step_size * 10  # 增加10倍步长
        
        # 使用三种不同的提议策略随机选择
        proposal_type = np.random.choice(3)
        
        if proposal_type == 0:
            # 方法1：基于当前状态的Dirichlet提议
            alpha = current_ratios / effective_step + 0.1
            new_ratios = np.random.dirichlet(alpha)
            
        elif proposal_type == 1:
            # 方法2：添加大的随机扰动
            perturbation = np.random.normal(0, effective_step, len(current_ratios))
            new_ratios = current_ratios + perturbation
            new_ratios = np.maximum(new_ratios, 1e-6)
            new_ratios = new_ratios / np.sum(new_ratios)
            
        else:
            # 方法3：完全随机重采样（用于跳出局部最优）
            new_ratios = np.random.dirichlet(np.ones(len(current_ratios)))
