...
class MGM_RF_Pipeline:
    ...
    def analyze_sample(self, sample_data: pd.DataFrame, att2_freq_data: Dict[str, List[str]] = None) -> Dict:
        ...
        # 步骤1: 从文件名提取真实NoC（优先使用真实值）
        true_contributor_ids, true_ratios = self.mgm_rf_inferencer.q1_feature_engineering.extract_true_mixture_info(sample_file)
        
        if true_contributor_ids and len(true_contributor_ids) > 0:
            # 使用真实的贡献者人数
            true_noc = len(true_contributor_ids)
            noc_confidence = 1.0  # 真实值的置信度为1
            logger.info(f"样本 {sample_file} 真实贡献者人数: {true_noc}")
            logger.info(f"真实贡献者ID: {true_contributor_ids}")
            if true_ratios:
                logger.info(f"真实混合比例: {true_ratios}")
        
            # 仍然预测NoC用于对比
            predicted_noc, predicted_confidence, v5_features = self.mgm_rf_inferencer.predict_noc_from_sample(sample_data)
            logger.info(f"模型预测NoC: {predicted_noc} (置信度: {predicted_confidence:.3f}) vs 真实NoC: {true_noc}")
            
            # 使用真实NoC进行分析
            analysis_noc = true_noc
            
        else:
            # 如果无法从文件名提取，则使用模型预测
            predicted_noc, noc_confidence, v5_features = self.mgm_rf_inferencer.predict_noc_from_sample(sample_data)
            logger.warning(f"样本 {sample_file} 无法提取真实NoC，使用预测值: {predicted_noc}")
            
            true_noc = None
            analysis_noc = predicted_noc
            predicted_confidence = noc_confidence
        ...
        if sample_peaks.empty:
            ...
            return self._get_default_result(sample_file, analysis_noc, noc_confidence, v5_features, 
                                          true_contributor_ids, true_ratios, true_noc)
        ...
        logger.info(f"样本 {sample_file} 包含{len(observed_data)}个有效位点，使用NoC={analysis_noc}进行分析")
        
        # 步骤5: MCMC推断混合比例
        start_time = time.time()
        mcmc_results = self.mgm_rf_inferencer.mcmc_sampler(
            observed_data, analysis_noc, att2_freq_data)
        end_time = time.time()
        
        # 步骤6: 生成后验摘要
        posterior_summary = self.generate_posterior_summary(mcmc_results, analysis_noc, true_ratios)
        
        # 步骤7: 收敛性诊断
        convergence_diagnostics = self.analyze_convergence(mcmc_results['samples'], analysis_noc)
        
        result = {
            'sample_file': sample_file,
            'true_noc': true_noc,                    # 真实NoC
            'analysis_noc': analysis_noc,            # 分析使用的NoC
            'predicted_noc': predicted_noc if 'predicted_noc' in locals() else analysis_noc,  # 模型预测的NoC
            'noc_confidence': noc_confidence,
            'noc_prediction_accuracy': (predicted_noc == true_noc) if (true_noc and 'predicted_noc' in locals()) else None,
    ...
    def _get_default_result(self, sample_file: str, analysis_noc: int, noc_confidence: float, 
                           v5_features: Dict, true_contributor_ids: List = None, 
                           true_ratios: List = None, true_noc: int = None) -> Dict:
        """获取默认结果（当峰数据为空时）"""
        # 创建默认的混合比例
        if analysis_noc == 1:
            default_ratios = [1.0]
        else:
            default_ratios = [1.0/analysis_noc] * analysis_noc
        
        default_summary = {}
        for i in range(analysis_noc):
        ...
        return {
            'sample_file': sample_file,
            'true_noc': true_noc,
            'analysis_noc': analysis_noc,
            'predicted_noc': analysis_noc,
            'noc_confidence': noc_confidence,
            'noc_prediction_accuracy': None,
            'v5_features': v5_features,
            'mcmc_results': None,
    ...
    def generate_posterior_summary(self, results: Dict, N: int, true_ratios: List = None) -> Dict:
        """生成后验分布摘要统计"""
        ...
        # 计算与真值的比较（如果有真值）
        if true_ratios and len(true_ratios) > 0:
            ...
            if len(true_ratios) == len(predicted_means):
                ...
                # 计算相关系数
                correlation = np.corrcoef(predicted_means, true_ratios)[0, 1] if len(predicted_means) > 1 else 1.0
                ...
                summary['accuracy_metrics'] = {
                    ...
                    'absolute_errors': [abs(p - t) for p, t in zip(predicted_means, true_ratios)]
                }
            else:
                logger.warning(f"真实比例数量({len(true_ratios)})与预测比例数量({len(predicted_means)})不匹配")
...
def main():
    ...
    if choice == "1":
        ...
        try:
            ...
            all_results = analyze_all_samples_from_att(
                config.ATTACHMENT2_PATH,
                config.Q1_MODEL_PATH,
                max_samples
            )
