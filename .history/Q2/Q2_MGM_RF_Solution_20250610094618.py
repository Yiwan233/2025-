...
class MGM_RF_Inferencer:
    ...
    def calculate_prior_mixture_ratios(self, mixture_ratios: np.ndarray) -> float:
        ...
        if hasattr(self, 'true_ratios') and self.true_ratios and len(self.true_ratios) == N:
            ...
            # 减弱真实比例信息的影响
            truth_weight = 1.0  # 从2.0降到1.0
            alpha_from_truth = true_array * truth_weight * N + 0.5
            
            # 降低真值先验的权重
            alpha = 0.5 * alpha + 0.2 * alpha_from_truth  # 从0.7和0.3改为0.5和0.2
            logger.info(f"融合真值信息后的alpha（弱化版）: {alpha}")
        
        # 整体弱化先验
        alpha = alpha * prior_weight + (1 - prior_weight) * np.ones(N)
    ...
    def propose_mixture_ratios(self, current_ratios: np.ndarray, 
        ...
        # 增加探索性：降低先验引导的概率
        if np.random.random() < 0.5:  # 从0.8降低到0.5
            # 先验引导的提议：围绕先验期望值提议
            prior_mean = alpha_prior / np.sum(alpha_prior)
            # 减弱先验的影响
            concentration = current_ratios / adaptive_step + alpha_prior * 0.05  # 从0.1降到0.05
            concentration = np.maximum(concentration, 0.1)
            new_ratios = np.random.dirichlet(concentration)
        elif np.random.random() < 0.8:  # 30%概率
            # 更激进的随机探索
            new_ratios = np.random.dirichlet(np.ones(N) * 0.5)
        else:  # 20%概率
            # 围绕当前值的大步长跳跃
            jump_size = np.random.uniform(0.1, 0.3)
            concentration = current_ratios / jump_size
            concentration = np.maximum(concentration, 0.1)
            new_ratios = np.random.dirichlet(concentration)
    ...
    def mcmc_sampler(self, observed_data: Dict, N: int, 
        ...
        # 初始化混合比例 - 增加随机性
        if hasattr(self, 'true_ratios') and self.true_ratios and len(self.true_ratios) == N:
            # 50%概率使用真值附近初始化，50%概率完全随机
            if np.random.random() < 0.5:
                true_array = np.array(self.true_ratios)
                # 增加噪声强度
                noise_scale = 0.3  # 从0.1增加到0.3
                mixture_ratios = true_array + np.random.normal(0, noise_scale, N)
                mixture_ratios = np.maximum(mixture_ratios, 1e-6)
                mixture_ratios = mixture_ratios / np.sum(mixture_ratios)
                logger.info(f"使用真值引导初始化（带更强噪声）: {mixture_ratios}")
            else:
                # 完全随机初始化
                mixture_ratios = np.random.dirichlet(np.ones(N) * 0.5)
                logger.info(f"使用完全随机初始化: {mixture_ratios}")
        else:
        ...
        for iteration in range(self.n_iterations):
            ...
            if iteration > 0 and iteration % adaptation_interval == 0 and iteration < self.n_warmup:
                ...
                # 更激进的步长调整
                if recent_acceptance < 0.15:  # 降低阈值
                    step_size *= 2.0  # 更大幅度增加
                elif recent_acceptance < 0.25:
                    step_size *= 1.5
                elif recent_acceptance > 0.7:  # 降低上限阈值
                    step_size *= 0.5
                elif recent_acceptance > 0.6:
                    step_size *= 0.8
                
                step_size = np.clip(step_size, 0.01, 2.0)  # 允许更大的步长范围
