...
for i, param in enumerate(key_params):
    ...
    try:
        ...
        # 修复：使用正确的置信区间透明度和颜色
        axes[i].fill_between(x_axis, train_mean - train_std, train_mean + train_std, 
                            alpha=0.1, color='blue', label='训练集±1σ')
        axes[i].fill_between(x_axis, validation_mean - validation_std, validation_mean + validation_std, 
                            alpha=0.1, color='red', label='验证集±1σ')
        ...
        if optimal_value is not None:
            try:
                if param == 'max_depth' and optimal_value is None:
                    # 对于max_depth=None的特殊处理
                    optimal_idx = len(param_range) - 1 if None in param_range else -1
                elif optimal_value in param_range:
                    optimal_idx = param_range.index(optimal_value)
                else:
                    # 如果最优值不在范围内，找最接近的
                    if isinstance(optimal_value, (int, float)):
                        numeric_params = [p for p in param_range if isinstance(p, (int, float))]
                        if numeric_params:
                            closest_value = min(numeric_params, key=lambda x: abs(x - optimal_value))
                            optimal_idx = param_range.index(closest_value)
                        else:
                            optimal_idx = -1
                    else:
                        optimal_idx = -1
