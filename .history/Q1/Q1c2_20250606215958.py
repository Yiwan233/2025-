...
def extract_enhanced_features(sample_file, sample_peaks):
    ...
    if len(locus_groups) > 0:
        # 位点高度分布特征
        locus_height_stats = {
            'mean': locus_mean_heights.values,
            'max': locus_max_heights.values,
            'min': locus_min_heights.values,
            'std': locus_std_heights.fillna(0).values
        }
        ...
        # 位点效率指标
        features['最高效位点高度'] = locus_heights.max()
        features['最低效位点高度'] = locus_heights.min()
