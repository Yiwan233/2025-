...
⚠️ 重要提醒：请将此文件保存为 excel_to_csv.py 或 xlsx_converter.py
不要命名为 csv.py，这会与Python标准库冲突！
...
使用示例:
  # 转换单个文件
  python excel_to_csv.py -f data.xlsx
  
  # 转换文件的所有工作表
  python excel_to_csv.py -f data.xlsx --all-sheets
  
  # 批量转换目录中的所有Excel文件
  python excel_to_csv.py -d ./excel_files/ --batch
  
  # 指定输出目录和编码
  python excel_to_csv.py -f data.xlsx -o ./output/ --encoding utf-8
        """
    )
