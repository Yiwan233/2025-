...
def extract_comprehensive_features_v5(sample_file, sample_peaks):
    ...
    if total_peaks > 0:
        ...
        # 新增：添加特征平滑
        features['avg_peak_height'] = features['avg_peak_height'] * 0.95 + 1000 * 0.05  # 向全局均值回归
        features['std_peak_height'] = min(features['std_peak_height'], features['avg_peak_height'] * 0.5)  # 限制标准差
...
# 在特征选择部分，增加稳定性筛选
def select_robust_features(X, y, selected_features):
    """选择在不同数据子集上表现稳定的特征"""
    ...
    for feature in selected_features:
        ...
        for train_idx, val_idx in kf.split(X):
            ...
            corr = abs(X_train_fold[feature].corr(y_train_fold))
            if not np.isnan(corr):
                correlations.append(corr)
        
        # 只保留相关性稳定的特征
        if len(correlations) > 0 and np.std(correlations) < 0.1:
            stable_features.append(feature)
    
    return stable_features[:12]  # 限制特征数量
    ...
    for class_label in np.unique(y):
        ...
        if len(class_df) < max_size:
            class_df_upsampled = resample(class_df, 
                                        replace=True,
                                        n_samples=min(max_size, len(class_df) * 2),  # 限制上采样倍数
                                        random_state=42)
...
# 应用数据平衡
X_final_balanced, y_encoded_balanced = balance_dataset(X_final, y_encoded)
print(f"平衡后类别分布: {Counter(y_encoded_balanced)}")
...
# 超参数网格
# 修改Gradient Boosting参数，增加正则化
gb_param_grid = {
    'n_estimators': [50, 100, 150],  # 减少估计器数量
    'max_depth': [1,2,3, 4, 5,6,7,8,9],          # 限制树深度
    'learning_rate': [0.1, 0.05,0.01,0.005,0.0001,0.00005],    # 降低学习率
    'subsample': [0.7,0.8, 0.9,0.95],         # 增加子采样
    'min_samples_split': [5, 10, 15], # 增加最小分割样本数
    'min_samples_leaf': [3, 5, 8],    # 增加叶节点最小样本数
    'max_features': ['sqrt', 0.5]     # 新增：限制特征数量
}

# 增加早停机制
best_gb_model = GradientBoostingClassifier(
    random_state=42,
    validation_fraction=0.2,  # 新增：用于早停的验证集比例
    n_iter_no_change=10,      # 新增：早停轮数
    tol=1e-4                  # 新增：早停容忍度
)
...
# 如果有样本分组信息，使用分组交叉验证
# 这里假设样本文件名可以用作分组依据
def create_sample_groups(df_features):
    """基于样本文件创建分组"""
    # 提取样本的基础ID（去除重复标识）
    df_features['sample_group'] = df_features['Sample File'].str.extract(r'([^-]+)')[0]
    return df_features['sample_group']

# 创建分组交叉验证
sample_groups = create_sample_groups(df_features)
...
# 修改集成模型，增加多样性
ensemble_models = [
    ('gb_shallow', GradientBoostingClassifier(max_depth=3, n_estimators=50, random_state=42)),
    ('gb_deep', GradientBoostingClassifier(max_depth=6, n_estimators=100, random_state=123)),
    ('rf', RandomForestClassifier(n_estimators=100, max_depth=8, random_state=42, class_weight='balanced')),
    ('et', ExtraTreesClassifier(n_estimators=100, max_depth=6, random_state=42, class_weight='balanced'))
]

ensemble_model = VotingClassifier(
    estimators=ensemble_models,
    voting='soft',
    weights=[2, 1, 2, 1]  # 给浅层模型更高权重
)
