...
print(f"选择特征后训练集形状: {X_train_selected.shape}")
print(f"选择特征后测试集形状: {X_test_selected.shape}")# -*- coding: utf-8 -*-
"""
...
base_estimator = GradientBoostingClassifier(
    ...
    learning_rate=0.1,
    random_state=42,
    validation_fraction=0.0  # 禁用内部验证以避免分割问题
)
...
print(f"最小类别样本数: {min_class_size}")
...
# 使用自定义分层划分（在数据平衡性处理之后）
X_train, X_test, y_train, y_test = custom_stratified_split(X_scaled, y_encoded, test_size=0.25)
...
if imbalance_ratio > 5:
    print("⚠️ 检测到严重类别不平衡，建议使用重采样方法")
    try:
        from imblearn.over_sampling import SMOTE
        print("使用SMOTE进行过采样...")
        smote = SMOTE(random_state=42, k_neighbors=min(3, min_samples-1))
        X_scaled_balanced, y_balanced = smote.fit_resample(X_scaled, y_encoded)
        print(f"SMOTE后样本数: {len(X_scaled_balanced)}")
        
        # 更新后续使用的数据
        X_scaled = X_scaled_balanced
        y_encoded = y_balanced
        
    except ImportError:
        print("未安装imblearn，跳过SMOTE处理")
        print("可通过 pip install imbalanced-learn 安装")

print(f"最终用于建模的数据形状: {X_scaled.shape}")
print(f"最终类别分布: {pd.Series(y_encoded).value_counts().sort_index().to_dict()}")

# 重新计算最小类别大小
min_class_size = pd.Series(y_encoded).value_counts().min()
print(f"更新后最小类别样本数: {min_class_size}")

# 设置交叉验证
cv_folds_final = min(5, min_class_size)
cv = StratifiedKFold(n_splits=cv_folds_final, shuffle=True, random_state=42)
print(f"最终模型使用{cv_folds_final}折交叉验证")
...
# 粗网格搜索
coarse_gb = GradientBoostingClassifier(
    random_state=42, 
    validation_fraction=0.0,  # 禁用内部验证
    warm_start=False
)
...
coarse_grid_search = RandomizedSearchCV(
    ...
    coarse_param_grid,
    n_iter=20,  # 进一步减少搜索次数
    cv=min(3, min_class_size),  # 动态调整CV折数
...
# 拟合时使用样本权重
coarse_grid_search.fit(X_train_selected, y_train, sample_weight=class_weights)
...
# 精细网格搜索
fine_gb = GradientBoostingClassifier(
    random_state=42, 
    validation_fraction=0.0,  # 禁用内部验证
    warm_start=False
)
...
fine_grid_search.fit(X_train_selected, y_train, sample_weight=class_weights)
...
# 计算平衡准确率
balanced_acc = balanced_accuracy_scorer(best_gb_model, X_test_selected, y_test)
