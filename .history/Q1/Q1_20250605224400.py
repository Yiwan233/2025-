...
# =====================
print("\n" + "="*80)
print("                  法医混合STR图谱NoC智能识别系统")
print("                         最终分析报告")
print("="*80)

print(f"\n📊 数据概况:")
print(f"   • 样本总数量: {len(df_features):,} 个")
print(f"   • 贡献者人数分布: {dict(df_features['NoC_True'].value_counts().sort_index())}")
print(f"   • 原始特征数量: {len(feature_cols):,} 个")
print(f"   • 精选特征数量: {len(selected_features):,} 个")
print(f"   • 特征筛选比例: {len(selected_features)/len(feature_cols)*100:.1f}%")

# 翻译最佳模型名称
best_model_name_cn = best_model_name
if best_model_name == 'Gradient Boosting':
    best_model_name_cn = '梯度提升树'
elif best_model_name == 'Random Forest':
    best_model_name_cn = '随机森林'
elif best_model_name == 'Ensemble':
    best_model_name_cn = '集成模型'

print(f"\n🏆 最优模型: {best_model_name_cn}")
print(f"   • 测试集准确率: {best_accuracy:.4f} ({best_accuracy*100:.1f}%)")
print(f"   • 整体准确率: {overall_accuracy:.4f} ({overall_accuracy*100:.1f}%)")
...
if best_model_name == 'Gradient Boosting':
    ...
    param_translations = {
        ...
        'n_iter_no_change': '早停轮数'
    }
    for param, value in params.items():
        param_cn = param_translations.get(param, param)
        print(f"     - {param_cn}: {value}")
...
if hasattr(best_model, 'feature_importances_'):
    ...
    for i, (_, row) in enumerate(top_5_features.iterrows(), 1):
        feature_cn = feature_name_mapping.get(row['feature'], row['feature'])
        print(f"   {i}. {feature_cn:<20} (重要性: {row['importance']:.4f})")

print(f"\n📋 模型特点说明:")
print(f"   • 基于 {len(selected_features)} 个精选生物特征进行预测")
print(f"   • 特征涵盖: 图谱统计特性、峰高分布特征、位点平衡性、")
print(f"     信息熵指标、DNA降解标志等多个维度")
print(f"   • 采用 {best_model_name_cn} 算法，针对少数类样本进行特殊优化")
print(f"   • 整体预测准确率达到 {overall_accuracy:.1%}，具有较好的实用价值")

# 数据质量评估
print(f"\n📊 数据质量评估:")
noc_distribution = df_features['NoC_True'].value_counts().sort_index()
...
if low_performance_classes:
    ...
    print(f"     - 增加训练样本数量")
    print(f"     - 调整样本权重或使用过采样技术")
    print(f"     - 考虑增加针对性特征")
else:
    print(f"   • ✅ 所有类别性能均达到良好水平")

print(f"\n💾 输出文件:")
print(f"   • 特征数据文件: noc_features_with_predictions.csv")
print(f"   • 模型性能摘要: noc_analysis_summary.json")
print(f"   • 训练好的模型: best_noc_model_{best_model_name.lower().replace(' ', '_')}.pkl")
